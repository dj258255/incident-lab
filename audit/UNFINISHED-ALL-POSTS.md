...
C가 42%입니다. 포트폴리오에서 가장 아픈 비중이고, 그래서 아래 C 목록을 우선순위대로
처리합니다.
...
backup-pitr의 Oracle과 SQL Server 대조입니다. Oracle은 공식 컨테이너 이미지가 라이선스
동의를 요구하고 SQL Server는 이미지는 있으나 PITR(`STOPAT`) 실습에 Enterprise 기능이
걸립니다. **문서 대조까지는 가능하므로 그 형태로 글에 넣었습니다.**

### B7. 나머지 (10건)

perf-insights-clone의 실제 PI 화면 대조(계정 필요), index-not-used의 두 호스트 값 비교
(장비가 이미 다름), 원 사건 수치와의 직접 비교(규모가 다름) 등입니다.
