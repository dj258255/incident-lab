...
8. **끝나면 검산한다.** 회수 합계가 대상 합계와 다르면 배치를 실패로 남기고 던진다.
...
- **NEGATIVE 설계의 위험을 재지 않았습니다.** 음수 잔액을 읽는 코드가 어떻게 깨지는지는
  ...
  없습니다.
- **승인자를 여러 명 요구하는 구조는 안 만들었습니다.** 7절의 승인은 한 명이면 통과합니다.
  금액이 크면 두 명 이상을 요구하는 단계를 두는 것이 보통인데 그 자리는 비어 있습니다.
...
- **되돌리는 보정에 승인을 안 붙였습니다.** 8절의 되돌리기는 바로 실행합니다. 되돌리기도
  이용자 자산을 바꾸는 작업이므로 같은 승인 절차를 타야 합니다.
