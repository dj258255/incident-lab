...
| 지금 알면 되는 것 | 근거 |
|---|---|
| **4,000행 배치 커밋으로 승격 0회, 다섯째 배치에서 죽여도 이중 회수 0·누락 0.** 진행 지점을 회수와 같은 트랜잭션에서 옮겼기 때문이다 | ([근거](#4-중간에-죽으면)) |
| **회수 대상 6만 중 2만은 잔액이 회수액보다 적다.** 음수 잔액과 빚 컬럼 두 설계가 회수 총액은 같되 상계 방식이 갈린다 | Msg 547 / 총액 1,839,974,000 ([근거](#3-두-설계)) |
| **데드락을 엔진에 맡기면 9회 모두 게임 트래픽 쪽이 죽는다.** `DEADLOCK_PRIORITY LOW` 를 보정에 주자 3/0 으로 뒤집혔다 | ([근거](#13-회수-프로시저가-데드락을-견디는가)) |
| **잘못된 보정을 되돌릴 때 운영 DB 를 통째로 복원하면 안 된다.** 그러면 보정 뒤에 쌓인 정상 활동 3억이 함께 사라진다 | 복원본에서 원래 값만 읽어 보정을 한 번 더 ([근거](#8-운영-db-를-안-되돌리고-회수만-취소한다)) |
| **파일이 절반에서 잘려도 프로시저 검산은 통과한다.** 30건 받아 30건 회수. 원본과의 건수·합계·체크섬 대조만 이것을 잡는다 | ([근거](#14-이관의-두-구멍)) |
...
넘어가기는 넘어갑니다. 지문이 맞고 bcp·링크드 서버와 결과가 같습니다.
**데이터가 도착하는 것 자체는 세 방법이 다르지 않습니다.**

### 여기까지 오는 동안 세팅 단계가 두 번 조용히 죽었습니다
...
두 번째가 특히 고약합니다. `sp_addpushsubscription_agent` 에 `@publisher_security_mode`
를 넘겼는데 **그 프로시저에는 없는 매개변수**라 `EXEC` 전체가 실패했고, 같이 넘긴
`@subscriber_security_mode = 0` 도 통째로 안 걸렸습니다. 에이전트는
`sp_addsubscription` 이 만든 기본값인 **모드 1(통합 인증)** 으로 남았고, Linux 에는
Kerberos 가 없으니 접속할 때 `Cannot generate SSPI context` 로 죽었습니다.
...
이 한 줄에서 진짜 원인까지 가려면 표를 두 개 봐야 합니다.
`MSdistribution_history` 의 `comments` 는 요약이라 여기까지만 말하고,
원문은 `distribution.dbo.MSrepl_errors` 에 있습니다.
...
> 처음에는 원인을 **자체 서명 인증서**로 봤습니다. 구독자에 CN 을 맞춘 인증서를
> 만들고 `mssql-conf` 로 물린 다음 발행자의 신뢰 목록에 넣어 `sqlcmd` 가 `-C` 없이
> 붙는 것까지 확인했는데, **그래도 에이전트는 못 붙었습니다.** 반대로 인증 방식을
> 고친 뒤 그 신뢰를 도로 걷어냈더니 전달은 그대로 됐습니다. 인증서는 원인이
> 아니었습니다. `-EncryptionLevel 0` 이면 검증 자체를 안 합니다.
> **가설이 그럴듯하다고 원인인 것은 아닙니다.** 걷어내 보고서야 갈렸습니다.

그래서 이 실험은 각 단계를 **되읽어 확인**하도록 고쳤습니다(14단계). 단계가 많은
절차에서는 "실행했다"와 "걸렸다"가 다르고, 그 차이는 맨 끝에서 전혀 다른 얼굴로
나타납니다. **13단계짜리 세팅에서 이런 일이 두 번 났다는 것이, 사고 중에 이 도구를
쓰면 안 되는 이유입니다.**
...
## 17. INSTEAD OF 가 떠안는 것

11절에서 `INSTEAD OF` 는 "그 표에 오는 **모든** 갱신을 책임진다"고 적고 **그것을 안
보였습니다.** 주장만 있고 증거가 없었습니다. 지급만 다루는 트리거를 걸고 다른 갱신을
던집니다.
...
![INSTEAD OF 는 자기가 안 보는 갱신을 조용히 버린다](results/fig-instead-of-gap.png)

**오류가 하나도 안 났습니다.** 갱신이 성공했다고 돌려주고 아무 일도 안 일어납니다.
트리거가 다루는 조건(`balance` 가 늘어난 행)에 안 걸리면 그 갱신은 통째로 버려집니다.
등급을 바꾼 운영툴도, 메모를 남긴 사람도 반영된 줄 알고 넘어갑니다.

`AFTER` 는 자기가 볼 것만 보고(`UPDATE(balance)`) 아니면 물러납니다. **원래 갱신을
안 건드립니다.** 다른 경로가 추가돼도 트리거를 안 고쳐도 됩니다.

11절에서 `INSTEAD OF` 는 로그가 싸고(2.5MB) 락이 비쌌습니다(20,121개). 여기서 **세
번째 대가**가 나왔습니다. 표에 컬럼이 늘거나 갱신 경로가 추가될 때마다 트리거를 같이
고쳐야 하고, **안 고치면 그 경로가 조용히 죽습니다.** 앞 절의 복제와 같은 종류의
고장입니다. 실패가 실패처럼 안 보입니다.
...
### DB 가 클라이언트에 무엇을 보장할 수 있는가

음수 잔액을 읽는 **앱 코드**는 이 랩 밖입니다. 다만 DB 가 그 앱에 무엇을 알려 줄 수
있는지는 안에 있습니다. 제약은 **읽을 수 있는 메타데이터**입니다.
...
제약이 있으면 그 사실이 `sys.check_constraints` 에 남고 ORM·코드 생성기·스키마 문서화
도구가 전부 이것을 읽습니다. 제약을 빼면 클라이언트는 "음수가 올 수 있다"만 알게 되고,
그것을 어떻게 다룰지는 각 코드가 알아서 정합니다. **그 순간부터 통일된 규칙이
없어집니다.**

그래서 "클라이언트가 음수에 **어떻게 반응하는가**"는 DB 밖이지만, **"클라이언트가
음수를 예상해야 하는가"는 DB 가 정합니다.**

## 18. 음수 잔액을 읽는 쪽이 각각 어떻게 틀리는가

15절은 앱이 음수를 못 보게 뷰로 가렸고, **읽는 쪽이 각각 어떻게 틀리는지**는 앱
계층이라며 안 봤습니다. 그것도 선을 잘못 그은 것입니다. 음수가 깨뜨리는 것은 화면이
아니라 **쿼리**이고, 쿼리는 DB 안에 있습니다.
...
![음수 잔액은 읽는 쪽마다 다르게 틀린다](results/fig-negative-readers.png)

**틀리는 자리가 종류마다 다르고, 안 틀리는 자리도 있습니다.**

상점의 구매 가능 판정(`WHERE balance >= 가격`)은 세 설계가 같습니다. 음수든 0 이든
못 사는 것은 마찬가지라 부등호가 알아서 걸러 냅니다. **여기만 보고 "음수여도
괜찮다"고 판단하면 나머지를 놓칩니다.**

같은 상점 화면의 **부족액 안내**에서 바로 갈립니다. `가격 - 잔액` 으로 계산하면
NEGATIVE 에서는 회수당한 만큼이 얹혀 실제 필요액보다 부풀어 나옵니다(1,600 대 1,000).
이용자에게는 "빚부터 갚아야 살 수 있다"로 보입니다. 그것이 의도라면 맞고, 의도가
아니라면 **아무도 모르게 틀린 안내**가 나갑니다.

### 통계에서 갈리는 자리는 필터가 정합니다

여기는 예상과 달랐습니다. **보유자 평균(`WHERE balance > 0`)은 세 설계가 같습니다.**
NEGATIVE 에서는 음수가, DEBT 에서는 0 이 걸러지는데 그것이 **같은 사람들**이라 남는
모집단이 같기 때문입니다. 필터를 빼면 바로 갈립니다. 전체 평균은 2,600 대 2,800 입니다.

그래서 "통계가 틀린다"는 한 문장으로는 부족합니다. **어떤 지표는 맞고 어떤 지표는
틀리며, 그 경계는 각 쿼리의 `WHERE` 절이 정합니다.**

### 충전은 안 틀립니다

```
  계정 3 에 500 충전       NEGATIVE  DEBT       보호 뷰
  충전 뒤 (잔액 / 빚)      -100      0 / 100    0 / 100
```

음수에 더하는 것과 빚에서 빼는 것이 같은 값을 냅니다. **NEGATIVE 가 상계 로직을 안
써도 되는 것은 사실입니다.** 다만 그 대가를 안 치르고 얻는 값이 위의 표 전부입니다.

### 뷰는 읽기를 고칠 뿐 표를 안 고칩니다
...
배치 잡, 백오피스 쿼리, 다른 팀의 분석 도구, 리포트 하나라도 기저 표를 직접 읽으면
그 경로는 33,333개를 그대로 봅니다. 그리고 그런 경로가 있는지 없는지는 **DB 가 강제할
수 없습니다.** 기저 표에 `DENY SELECT` 를 걸어도 `sysadmin` 과 `db_owner` 는 지나갑니다.

**DEBT 설계는 이 질문 자체가 없습니다.** 표에 음수가 없으므로 어느 경로로 읽어도 음수가
안 나옵니다.

> **위험한 것은 "안 틀린다" 줄들입니다.** 제일 먼저 확인해 보는 상점의 구매 판정이
> 멀쩡하고, 통계도 보유자 평균만 보면 맞습니다. 거기서 "음수여도 괜찮다"고 결론 내면
> 나머지가 전부 틀린 채로 나갑니다. 그래서 "음수 잔액을 읽는 코드를 전부 확인한다"는
> 계획은 성립하기 어렵습니다. 확인해도 **틀린 것과 안 틀린 것이 섞여 나와** 통과했다는
> 착각을 줍니다.

## 19. 복제를 상시 구성으로 세우면 무엇이 더 붙는가

16절은 표 하나를 **한 번** 넘기는 비용을 쟀고, 결론에서 "복제를 쓸 자리는 조사 DB 를
운영에서 계속 떠먹이는 구성"이라고 적었습니다. **그 자리를 권해 놓고 그 구성은 안 세워
봤습니다.**

16절은 스냅샷 복제였습니다(`@repl_freq = 'snapshot'`). 상시 구성은 **트랜잭션 복제**고
그것부터 다릅니다.
...
![상시 구성은 세우는 비용보다 유지하는 비용이 크다](results/fig-replication-standing.png)

**로그 판독기가 새로 생긴 것입니다.** 발행 DB 의 트랜잭션 로그를 계속 읽어 바뀐 것을
배포 DB 로 옮깁니다. 이것이 있어야 증분이 따라옵니다.

> 이 에이전트는 발행별이 아니라 **발행 DB 별**이라 작업 이름에 발행명이 안 들어갑니다
> (`A25-REPL-PUB-repl_src-1`). 발행명으로 찾다가 "없음"으로 잘못 읽었습니다.
...
구독자를 다시 올리면 **알아서 따라잡습니다.** 미전달이 20,000건에서 0건으로 돌아오고
사람이 할 일은 없습니다. 다만 그것은 보존 기간(`@retention`, 기본 72시간) 안에서만
참입니다. 넘기면 구독이 만료되고 스냅샷부터 다시입니다.

**그런데 전달이 끝났는데도 배포 DB 는 안 줄었습니다.** 보관 중인 명령 20,546건과 86MB
가 그대로입니다. `@immediate_sync = true` 로 만든 발행이라 새 구독자가 스냅샷 없이
붙을 수 있게 명령을 보존 기간 내내 들고 있기 때문입니다. **배포 DB 크기는 구독자의
상태가 아니라 발행 설정과 변경량이 정합니다.**

### 로그 판독기가 멈추면 운영 DB 가 진다

여기가 상시 구성에서 제일 위험한 자리입니다. 같은 쓰기를 두 번, 한 번은 판독기가 도는
채로 한 번은 멈춘 채로 했습니다. 복구 모델은 둘 다 `SIMPLE` 입니다.
...
자른다고 알기 쉬운데, 복제가 걸린 DB 는 **로그 판독기가 읽어 가기 전까지 그 구간을 못
버립니다.** `log_reuse_wait_desc` 가 `REPLICATION` 으로 그것을 말합니다. 판독기를 다시
켜자 회수됐으니 원인이 판독기라는 것도 섭니다.
...
**상시 구성은 세우는 비용보다 유지하는 비용이 큽니다.** 그래서 16절의 결론에 조건을
하나 붙입니다. 조사 DB 를 상시로 떠먹이는 구성이 맞기는 하지만, **그것은 이관 문제를
없애는 대신 상시 감시 대상을 셋 늘리는 거래입니다.** 회수가 1년에 몇 번인지에 따라
답이 갈립니다.

## 20. 예상과 달랐던 점

### 음수 잔액이 통계를 틀리게 하는 줄 알았는데, 어떤 통계는 맞았습니다

NEGATIVE 설계를 쓰면 집계가 전부 틀어질 것이라고 예상했습니다. 실제로는 **보유자
평균이 두 설계에서 같았습니다**(4,200 = 4,200). `WHERE balance > 0` 이 NEGATIVE 에서는
음수를, DEBT 에서는 0 을 걸러내는데 그것이 같은 사람들이라 남는 모집단이 같습니다.

같은 표에서 필터만 뺀 전체 평균은 2,600 대 2,800 으로 갈렸습니다. **틀리는 경계를
정하는 것은 설계가 아니라 각 쿼리의 `WHERE` 절이었습니다.** 그래서 "확인해 보고
괜찮으면 쓴다"가 안 통합니다. 확인한 쿼리는 맞고 확인 안 한 쿼리가 틀립니다.

### 배포 DB 는 구독자가 살아나면 줄어들 줄 알았습니다

구독자가 죽어 있는 동안 쌓인 것이니 다시 붙어 다 밀어 넣으면 줄어들 것이라고
예상했습니다. 미전달은 20,000건에서 0건으로 돌아왔는데 **보관 중인 명령 20,546건과
86MB 는 그대로였습니다.**

`@immediate_sync = true` 로 만든 발행이라 새 구독자가 스냅샷 없이 붙을 수 있게 명령을
보존 기간 내내 들고 있습니다. **배포 DB 크기는 구독자의 상태가 아니라 발행 설정과
변경량이 정합니다.** 구독자가 멀쩡해도 쓰기가 많으면 그만큼 자랍니다.
...
- **보존 기간을 넘겨 구독이 만료되는 것까지는 못 봤습니다.** 19절에서 구독자를
  내렸다 올리면 알아서 따라잡는 것을 확인했고, 그것이 보존 기간(기본 72시간) 안에서만
  참이라는 것은 설정값으로만 적었습니다. **72시간을 실제로 흘려보내지는 않았습니다.**
  보존 기간을 분 단위로 줄여 재현할 수는 있지만, 그때 만료되는 것이 실제 72시간과
  같은 경로인지 확인할 방법이 이 랩에는 없습니다.
- **화면에 무엇이 보이는지는 안 봤습니다.** 18절에서 음수 잔액이 상점·랭킹·통계
  **쿼리**를 각각 어떻게 틀리는지까지 쟀습니다. 그 결과를 받은 클라이언트가 그것을
  어떻게 그리는지(-600 을 그대로 찍는지, 0 으로 접는지, 오류로 처리하는지)는 DB 밖
  이고 게임마다 다릅니다. **DB 가 정할 수 있는 데까지는 갔습니다.**

**되돌리는 작업이 새 사고가 된 공개 사례가 있다.** 리니지M 은 2021년 1월 기능을 롤백하고
인게임 재화로 환급했는데, **3월 22일 보상 재산정 과정에서 일부 이용자가 사용액의 수십 배를
받는 오류**가 났고 그것이 아이템 시세를 흔들었다. 1차 사고(기능 롤백)보다 2차 사고(보상 배치)가
경제에 더 큰 피해를 줬다. 이 세션이 "다섯째 배치에서 죽여도 이중 회수 0·누락 0"을 판정 기준으로
삼은 이유가 그것이다. **복구 배치의 멱등성과 검산이 없으면 복구가 다음 장애다.**

회수가 아예 불가능해진 쪽의 결말도 둘 있다. RuneScape 는 2026년 4월 복사된 재화가 이미 퍼져
선별 회수를 포기하고 **패치 1시간 전 시점으로 전체 롤백**했고, World of Warcraft 는 2024년 9월
길드 은행 소실에 대해 **"남은 소실 아이템을 복구할 방법이 우리에게 없다"** 고 공개적으로 인정했다.
