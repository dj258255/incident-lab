...
- **시간을 재지 않았습니다.** 이 랩은 ARM 에뮬레이션이라 6만 계정 회수가 몇 초
  걸리는지, 배치 크기를 바꾸면 총 시간이 얼마나 늘어나는지 답할 수 없습니다.
  **랩 조건 때문에 못 닫은 항목입니다.**
- **음수 잔액을 읽는 애플리케이션 코드는 안 봤습니다.** 10절은 DB 안에서 깨지는 것만
  쟀습니다. 클라이언트가 음수를 어떻게 표시하고 계산하는지는 이 랩 밖입니다.
- **복제로 넘기는 경우는 안 쟀습니다.** 14절은 bcp 와 링크드 서버 둘을 견줬고
  복제(replication)는 세팅 비용이 커서 만들지 않았습니다.
