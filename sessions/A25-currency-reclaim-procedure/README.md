...
- **음수 잔액을 읽는 클라이언트 코드는 안 봤습니다.** 15절과 17절은 DB 가 앱에
  무엇을 보장할 수 있는지(뷰로 가리기, 제약을 메타데이터로 남기기)까지 만들었습니다.
  그 방어를 안 걸었을 때 상점·랭킹·통계 코드가 각각 어떻게 깨지는지는 **애플리케이션
  계층이라 이 랩 밖입니다.** DB 쪽 절반은 닫혔고, 나머지 절반은 이 세션의 범위가
  아닙니다.
- **복제를 계속 도는 구성으로는 안 재 봤습니다.** 16절은 표 하나를 한 번 넘기는
  비용을 쟀습니다. 조사 DB 를 운영에서 상시로 떠먹이는 구성이라면 배포 지연·재시도·
  구독 재초기화 같은 것이 따로 붙는데, 그 쪽은 이 세션의 질문(일회성 이관에 맞는가)
  밖이라 세우지 않았습니다.
