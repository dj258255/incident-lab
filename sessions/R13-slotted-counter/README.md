...
- **복제를 구성하지 않았습니다.** 인스턴스는 2대로 늘려 봤지만 DB는 단일 노드입니다. 읽기를 복제본으로 돌리면 슬롯 합계는 복제 지연만큼 뒤처진 시점의 값이 됩니다. 뒤처지는 것 자체는 단일 행도 마찬가지지만, 슬롯을 꺼내는 이유가 원본의 쓰기 처리량을 올리는 것이라서 복제본이 따라가야 할 양도 같이 늘어납니다. 그 지연이 얼마나 벌어지는지는 재지 않았습니다. 복제 지연과 승격은 같은 트랙의 `R01-replica-promotion-loss` 세션에서 다룹니다.
