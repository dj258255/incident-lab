...
슬롯 16과 64 중 무엇을 고를지는 취향이 아니라 나눗셈으로 정합니다. 필요한 값은 두 개입니다. 한 행이 받아내는 상한과, 그 카운터 하나에 몰릴 것으로 보는 최대 부하입니다.
...
죽어 있던 구간과 날아간 카운터만큼 74,792건, 1억 6,670만 원이 비었지만, 원장 167,776건에서 카운터를 다시 만드는 데 331ms가 걸렸고 유실은 0이 됐습니다. 4절에서 "카운터는 파생 데이터"라고 적은 주장의 실행 증명입니다.
...
|---|---|---|---|---|
| 단일 행 | 초당 933.1건 | 1,574,800 | 2초 | **0.53초** |
| 슬롯 64 | 초당 2,841.9건 | 158,478,400 | 23초 | **41.83초** |

처리량은 3.0배인데 따라잡는 시간은 79배입니다. 이유가 두 겹입니다. 원본이 3배 쓰면
binlog 도 3배 나오고 복제본은 SQL 스레드 하나로 적용합니다. 그리고 슬롯은 같은 금액을 더
많은 행에 흩으므로 행 단위 이벤트 수 자체가 늘어납니다. **원본에서 이득이던 분산이
복제본에서는 부담이 됩니다.**

판정을 `Seconds_Behind_Source` 하나에 걸지 않았습니다. 단일 행 조건에서 2초로 찍혔는데
실제 따라잡기는 0.53초였습니다. 초 단위이고 SQL 스레드가 방금 적용한 이벤트의 타임스탬프로
계산되기 때문입니다.
...
5절의 2대 비교는 인스턴스마다 1GB 를 줘 총 힙이 2GB 였습니다. `TOTAL_HEAP_MB` 를 1,024 로
고정하고 다시 쟀습니다(2대는 512MB 씩).
...
이유는 락 획득 순서였습니다. 모든 트랜잭션이 `sponsor_log` INSERT를 먼저 하고 카운터 행을 나중에 잡습니다. 순서가 같으면 서로가 서로를 기다리는 고리가 생기지 않습니다. 데드락은 자원이 둘 이상이고 획득 순서가 엇갈릴 때 생깁니다. 한 행에 쓰기가 몰리는 것만으로는 대기가 길어질 뿐입니다.
...
`scripts/exp-uniform-and-repeat.sh`, 결과는 `results/exp-uniform-and-repeat.txt` 입니다.

7절은 zipf 부하 하나였습니다. 쏠린 부하에서만 재면 자동 조절이 **실제로 조절을 하는지,
아니면 그냥 항상 늘리는지** 갈리지 않습니다. 부하 스크립트에 `uniform` 시나리오를
넣고 문턱도 함께 바꿨습니다.

| 문턱 | 부하 | 배정된 슬롯 합 | **방송당 최대** | tps |
|---|---|---|---|---|
| 50 | uniform | 8,016 | **16** | 4,457 |
| 200 | zipf | 2,115 | **64** | 4,300 |
| 200 | uniform | 2,010 | **4** | 4,452 |
| 1000 | zipf | 1,281 | **64** | 4,358 |
| 1000 | uniform | 1,000 | **1** | 4,404 |

**슬롯 합만 보면 속습니다.** 문턱 200 에서 zipf 2,115 와 uniform 2,010 이 거의 같습니다.
비율로는 0.95배라 "조절이 안 된다"로 읽힐 값입니다.

**판정은 방송당 최대 슬롯입니다.** 같은 문턱 200 에서 zipf 는 어떤 방송이 상한 64개를
다 받고, uniform 은 최대 4개입니다. 문턱 1000 에서는 uniform 이 **1개**, 즉 자동 조절이
아예 안 걸립니다. 방송이 1,000개이므로 합이 1,000 인 것도 그 뜻입니다.

**자동 조절이 실제로 조절을 합니다.** 쏠린 방송에는 상한까지 주고 고른 부하에는
거의 안 줍니다. 합이 비슷한 것은 zipf 에서 소수가 64개씩 갖고 나머지가 1개씩인 반면
uniform 은 전부 4개씩이기 때문입니다. **분포가 완전히 다른데 합이 우연히 가깝습니다.**

문턱을 올릴수록 두 조건 다 슬롯이 줄어듭니다. uniform 은 50에서 16개, 200에서 4개,
1000에서 1개로 문턱에 반비례합니다. 코드가 누적 쓰기가 문턱을 넘을 때마다 슬롯을
두 배로 키우므로 예상대로입니다.

처리량은 네 조건이 4,300~4,457 tps 로 3.6% 안에 있습니다. **슬롯을 덜 주는 것이
처리량을 깎지 않습니다.** 고른 부하에서는 애초에 경합이 없어 슬롯이 필요 없습니다.

`zipf` 문턱 50 조건은 앱이 뜨지 않아 빠졌습니다.

## 못 한 것

- **부하 발생기가 앱, DB와 같은 호스트에 있습니다.** k6와 Spring Boot와 MySQL이 한 대에서 CPU를 나눠 씁니다. 절대 수치를 서비스 용량으로 읽으면 안 되고, 변형 간 비교로만 읽어야 합니다.
- **복제 대조를 한 번씩만 쟀습니다.** 6절의 0.53초와 41.83초는 각각 1회 실행이고 복제본이 하나입니다. 지연 복제와 병렬 적용(`replica_parallel_workers`)은 다루지 않았습니다. 복제 지연과 승격은 같은 트랙의 `R01-replica-promotion-loss` 세션에서 다룹니다.
- **`zipf` 문턱 50 조건이 빠졌습니다.** 위 절에서 그 조건만 앱이 뜨지 않았습니다. 그리고 조건마다 1회 실행입니다.
- **5절 표의 2대 값은 총 힙 2GB 에서 잰 것입니다.** 8절에서 고정해 다시 쟀고 결론은 같았습니다.
