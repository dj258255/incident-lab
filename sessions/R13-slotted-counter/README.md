...
슬롯 카운터는 이 전제를 바꿉니다. 방송 하나의 카운터를 슬롯 N개로 쪼개면 같은 방송에 몰린 후원도 서로 다른 행을 건드립니다. 한 행 앞에 서 있던 줄이 N개의 줄로 흩어지므로, 뒤에 선 요청이 기다려야 하는 앞사람 수가 줄어듭니다. 실측에서 이 효과는 대기 횟수보다 대기 시간에서 훨씬 크게 나타났는데, 그 수치는 5절에 있습니다.
...
| 슬롯 카운터 | 예 | 읽을 때 `SUM`이 필요하고 테이블이 하나 늘어납니다 | 한 대상에 쓰기가 몰리는 것이 확실할 때 |
| Redis 카운터 | 조건부 | 원장과 카운터가 서로 다른 저장소에 있어 반영 주기만큼 어긋납니다 | 실시간 노출이 중요하고 약간의 지연을 허용할 때 |
...
처리량은 결과이고 락 대기가 원인입니다. 측정 구간의 `Innodb_row_lock_waits`와 `Innodb_row_lock_time` 증가분으로 한 번의 대기가 얼마나 길었는지를 계산했습니다.

![락 평균 대기](results/chart-lock-zipf.png)

슬롯의 효과는 대기 횟수 감소가 아닙니다. 슬롯 16은 대기 횟수(28,318회)가 원자적 UPDATE(25,367회)보다 오히려 많습니다. 줄어든 것은 한 번의 대기 시간입니다. 131ms에서 8ms로, 한 행 앞에 서 있던 줄이 16개로 흩어지면서 줄 하나가 짧아진 것입니다.
...
- **복제를 구성하지 않았습니다.** 인스턴스는 2대로 늘려 봤지만 DB는 단일 노드입니다. 읽기를 복제본으로 돌릴 때 슬롯 합계가 복제 지연만큼 뒤처지는 문제는 다루지 않았습니다.
- **슬롯 수를 자동으로 정하지 않습니다.** 16과 64를 재 봤을 뿐, 방송 인기도에 따라 슬롯 수를 조절하는 방식은 만들지 않았습니다.
- **인스턴스를 2대로 늘릴 때 총 힙이 2GB가 됩니다.** 1대(1GB)와 비교할 때 처리량 차이에 이 부분이 섞여 있습니다. 이 비교에서 읽어야 할 것은 처리량이 아니라 정합성 결과입니다.
