...
| 지금 알면 되는 것 | 근거 |
|---|---|
| **실패율 0%인데 후원의 30.9%가 사라진다.** 갱신 유실은 에러를 안 낸다 | 아홉 변형의 처리량·정합성 동시 측정 ([근거](#2-재현)) |
| **슬롯 카운터로 정합성과 처리량을 함께 얻는다** | ([근거](#5-재계측)) |
| **슬롯의 값은 복제본이 낸다.** 슬롯 64 는 원본 처리량 3.0배인 대신 복제본 따라잡기가 79배 | 0.53초 대 41.83초 ([근거](#슬롯의-값을-복제본이-냅니다)) |
| **자동 슬롯은 행 수를 11.4배 줄이면서 처리량을 92.5% 유지한다** | 방송별 쓰기 200건마다 두 배 ([근거](#7-슬롯-수를-자동으로-정하기)) |
| **인스턴스 비교에서 힙 혼입을 없애자 결론은 같고 읽을 수 있는 값이 됐다** | 처리량 16.9% 증가 ([근거](#8-인스턴스-비교에서-힙-혼입을-없앰)) |
