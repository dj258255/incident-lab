...
- 증거금 파라미터: 계약승수 1,000배럴, 하우스 증거금 8,000달러/계약, 계좌 자본 100,000달러, 유지증거금은 요구증거금의 75%. 이 가운데 하우스 증거금만 CFTC 명령서에 근거가 있습니다. 원문은 "On April 20, 2020, the initial house margin for long WTI was $8,462.50 per contract." 이고, 이 재현은 그 값을 8,000달러로 반올림해 썼습니다. 나머지 셋은 명령서에 없는 이 세션의 값입니다.
- 8,462.50달러를 그대로 넣으면 붕괴 분기점이 8.00달러가 아니라 8.4625달러, 기준 수량이 12계약이 아니라 11계약이 되고 배수는 그만큼 커집니다(0.10달러에서 90.91배, 0.01달러에서 909.09배). 분기점 아래 구간은 min이 명목가치를 고르므로 1,000계약·10,000계약과 과대계상 폭은 그대로입니다. 이 값들은 다시 실행해 잰 것이 아니라 같은 식에 대입해 계산한 것이고, 아래 기록은 전부 8,000달러로 실행한 결과입니다.
- 컨테이너 자원 한도(`cpus`, `mem_limit`)와 JVM 힙(`-Xmx`)을 지정하지 않았습니다. 이 기록에 시간 수치가 없어 그대로 두었고, 따라서 어떤 값도 성능 수치로 읽으면 안 됩니다.
- 세 계층은 서로 연결돼 있지 않습니다. `sql` 컨테이너와 `app` 컨테이너가 분리돼 있고 `app/NegativePrice.java`에 JDBC가 없습니다. 저장 계층이 남긴 최종가 0.01은 자바 쪽에 `STALE = 0.01` 상수로 옮겨 적은 것이라, 계층 간 전파는 재현이 아니라 두 실행 결과를 이어 붙인 서술입니다.
