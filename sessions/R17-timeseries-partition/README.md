...
|---|---|---|
| 소요 시간 | 20.5초 | **0.12초** |
| 삭제 중 INSERT p95 | 10.8ms (평상시 5.2ms의 2.1배) | 5.7ms (변화 없음) |
| 삭제 후 파일 크기 | 0.70GB → 0.70GB | 0.77GB → 0.42GB |

![디스크 회수](results/chart-disk.png)

행 기준으로 절반을 지웠는데 DELETE 쪽 파일은 1바이트도 줄지 않았습니다. 이 공간은 같은 테이블의 새 행이 재사용할 수는 있지만 OS로 돌아가지는 않습니다. 디스크 사용률 경보가 울리는 상황이라면 DELETE는 답이 되지 않는다는 뜻입니다.
...
| B: `DROP PARTITION` (LOCK=NONE) | `Waiting for table metadata lock` | 23.7초 |
...
청크 삭제는 총 46초로 한 방 DELETE(20.5초)의 2.2배였습니다. 청크마다 인덱스를 다시 찾아 들어가는 비용이 반복되기 때문입니다. 쪼개기의 목적은 총 시간 단축이 아니라 락 유지 시간과 트랜잭션 크기를 줄이는 것이고, 총 시간은 오히려 대가로 치릅니다.

### DROP PARTITION의 피해가 문장 뒤에 왔습니다

DROP 자체는 0.12초였고 그 구간의 INSERT p95는 5.7ms로 흔들리지 않았습니다. 그런데 25초쯤 지난 뒤 약 30초간 p95가 15~18ms로 오르는 구간이 나타났습니다(차트의 35~62초 구간). 파일 7개 삭제에 따른 버퍼 풀 정리나 백그라운드 플러시로 추정하지만 원인을 계측으로 특정하지는 못했습니다. 추정과 실측을 구분해 남깁니다.

### 범위 조건의 프루닝에 첫 파티션이 끼어 있었습니다

`created_at >= '2026-07-27'`의 프루닝 결과가 p20260727과 pmax만이 아니라 **남은 파티션 중 가장 낮은 p20260722를 포함한 3개**였습니다. 파티션 키 컬럼이 NULL인 행은 가장 낮은 파티션에 저장되므로, 범위 비교로는 그 파티션을 제외할 수 없기 때문입니다. NOT NULL 컬럼인데도 그렇습니다. 프루닝 수를 셀 때 하나가 더 나온다고 놀랄 일은 아니지만, 모르고 보면 프루닝이 덜 된다고 오해하기 좋습니다.
