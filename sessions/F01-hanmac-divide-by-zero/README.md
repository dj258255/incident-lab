...
| 지금 알면 되는 것 | 근거 |
|---|---|
| **NaN 은 상하한 비교를 뚫는다.** 비교 연산이 전부 false 라 "범위를 벗어나지 않았다"로 통과한다. Infinity 는 상한에 걸려 거부된다 | 주문 120건 중 NaN 10건 접수 ([근거](#2-재현)) |
| **유한성 가드 단독으로 비정상 접수가 0건이 된다** | Infinity 10 + NaN 10 전부 거부 ([근거](#5-재계측)) |
| **킬스위치는 뒤늦고 정상 주문까지 함께 막는다.** 섞인 배치에서 19건째에 발동해 정상 84건을 차단했다 | ([근거](#5-재계측)) |
| **가드에 구멍이 하나만 있어도 전부 통과한다.** 아무 일도 없던 것처럼 보이는 것이 이 결함의 성질이다 | E2 조건에서 NaN 20건 전부 접수 ([근거](#가드에-구멍이-있을-때와-임계값-스윕)) |
| **동시성 1에서도 이미 실제 사건의 속도를 넘는다** | ([근거](#8-시간축을-붙였습니다-초당-몇-건이-잘못-접수되는가)) |
...
## 가드에 구멍이 있을 때와 임계값 스윕

결과는 `results/spring-killswitch.txt` 입니다.

### 킬스위치가 무엇을 건지는가

앞의 조건들은 가드가 완전한 화이트리스트(`Double.isFinite`)라 NaN도 Infinity도 다
잡습니다. 실무의 가드는 자주 불완전합니다. `isInfinite` 만 확인하거나 절댓값으로
상한을 보면 Infinity는 걸리는데 **NaN은 두 비교가 모두 false 라 그대로 지나갑니다.**

그 조건에서 킬스위치의 탐지원이 가드와 같은지 다른지로 결과가 갈립니다.

| 엔드포인트 | 가드 | 킬스위치가 세는 것 |
|---|---|---|
| `/orders/leaky` | `isInfinite` 만 | 가드가 거부한 건수 |
| `/orders/leaky-independent` | 같음 | 접수 직전의 기준가 이탈 |

먼저 Infinity가 섞인 배치(정상 100 + Inf 10 + NaN 10)를 던졌습니다.

| 조건 | 201 접수 | 422 거부 | 503 킬스위치 |
|---|---|---|---|
| E. 가드 연동 | 100 (NaN 0) | 3 | 17 |
| F. 독립 탐지 | 100 (NaN 0) | 3 | 17 |

**두 조건이 같습니다.** 가드가 Infinity는 잡으므로 그때 킬스위치가 켜지고 뒤따르는
NaN이 막힙니다. **가드가 못 잡는 값을 킬스위치가 막은 것이 아니라 다른 종류의
비정상이 먼저 와서 대신 켜 준 것입니다.** 방어가 아니라 순서 운입니다.

그 우연을 걷어내려면 가드가 아무것도 못 잡는 배치가 필요합니다. 정상 100 + NaN 20만
던졌습니다.

| 조건 | 201 접수 | 422 거부 | 503 킬스위치 |
|---|---|---|---|
| E2. 가드 연동 | **120 (NaN 20)** | 0 | 0 |
| F2. 독립 탐지 | **100 (NaN 0)** | 3 | 17 |

**E2는 NaN 20건이 전부 접수됐습니다.** 킬스위치가 세는 것이 "가드가 거부한 건수"라,
가드가 못 잡은 값은 거부되지 않으니 킬스위치도 세지 못합니다. 422도 503도 0건입니다.
**아무 일도 일어나지 않은 것처럼 보입니다.**

**F2는 NaN 접수 0건입니다.** 탐지원을 접수 결과(기준가 이탈)로 옮기면 가드가 무엇을
놓치든 셀 수 있습니다.

**킬스위치의 값어치는 존재가 아니라 탐지원이 가드와 독립인가에서 나옵니다.**
가드가 거부한 건수만 세는 킬스위치는 가드의 사본이지 두 번째 방어선이 아닙니다.
이 세션이 이미 적어 둔 "비정상 접수 0건은 가드가 아니라 킬스위치가 만든 결과"라는
지적의 뒷면입니다. 가드가 완전하면 킬스위치가 일하고, 가드에 구멍이 있으면
**같은 구멍으로 킬스위치도 눈이 멉니다.**

### 임계값이 정하는 교환

임계값 3 하나만 썼습니다. 일곱 값을 재고 정상 주문의 부수 피해를 함께 봤습니다.
섞은 배치(정상 100 + 비정상 20)입니다.
...
**임계값 1은 정상 주문 100건을 전부 막습니다.** 첫 비정상이 오는 순간 접수가 닫히고
그 뒤 아무것도 못 들어갑니다.

**임계값 50 이상은 킬스위치가 아예 안 켜집니다.** 이 배치의 비정상이 20건뿐이라
문턱에 닿지 않습니다. 999와 결과가 같고, 그것이 곧 가드 단독 조건입니다.

가운데가 교환입니다. 임계값을 낮출수록 비정상은 빨리 멈추고 정상 주문의 부수 피해가
커집니다. **10에서 정상 52건이 막히고 20에서 11건이 막힙니다.**

**실제 값은 도메인이 정합니다.** 한맥은 2분 남짓에 3만 6천 건이 나갔습니다. 그 조건에서
부수 피해를 아끼려고 임계값을 높게 잡는 쪽이 훨씬 비쌌습니다. 반대로 정상 주문이
초당 수만 건인 시스템이라면 임계값 1은 자해입니다.

### 이 절의 한계

배치가 정상 100 대 비정상 20 이라 부수 피해의 크기가 그 비율에 매여 있습니다.
비정상 비율을 바꿔 가며 재지는 않았습니다. 조건마다 1회 실행입니다.

## 8. 시간축을 붙였습니다: 초당 몇 건이 잘못 접수되는가

여기까지의 재현에는 시간축이 없었습니다. 인프로세스 드라이버로 120건을 순차 호출한 것이라 "몇 건이 통과하는가"는 나오지만 "얼마나 빨리 쌓이는가"가 안 나옵니다. 한맥 사건의 핵심 수치가 143초에 37,900여 건이라는 속도인데, 그 축이 통째로 빠져 있었습니다.

앱을 서버 모드로 띄우고 k6로 동시성을 바꿔 가며 15초씩 걸었습니다. 대상은 7절의 `/orders/leaky` 입니다. 가드에 구멍이 있어 NaN이 통과하는 엔드포인트이고, 주문의 10%가 `days=0` 이라 NaN이 됩니다.

| 동시성 | NaN 접수 | 초당 | VU 1 대비 | p95 | 2분 환산 |
|---|---|---|---|---|---|
| 1 | 8,570 | 571.3/s | 1.0배 | 0.2ms | 68,560건 |
| 4 | 32,215 | 2,147.7/s | 3.8배 | 0.2ms | 257,720건 |
| 16 | 72,310 | 4,820.7/s | 8.4배 | 0.6ms | 578,480건 |
| 64 | 95,844 | 6,389.6/s | 11.2배 | 2.1ms | 766,752건 |

**동시성 1에서도 이미 사건의 속도를 넘습니다.** 한맥은 143초에 37,900여 건이었으니 초당 265건입니다. 이 랩은 스레드 하나로 초당 571건을 접수시킵니다. 물론 여기서 세는 것은 체결이 아니라 접수이고 호가 검증 한 겹만 흉내 낸 것이라 두 숫자를 같은 저울에 올리면 안 됩니다. 그래도 자릿수는 말해 줍니다. **사람이 화면을 보고 손으로 끄는 속도가 아닙니다.**

**차단이 네 조건 모두 0건입니다.** 킬스위치가 한 번도 안 걸립니다. 이 엔드포인트로 들어오는 것은 NaN 인데 킬스위치가 세는 것은 Infinite 뿐이기 때문입니다. 7절이 킬스위치의 값어치는 탐지원이 가드와 독립인가에서 나온다고 적은 자리가, 시간축 위에서 다시 나옵니다. **접수량이 동시성에 비례해 늘어나는 동안 계측기는 0을 유지합니다.**

동시성을 64배 올렸는데 접수는 11.2배만 늘었습니다. 2 vCPU 컨테이너라 CPU가 먼저 막힙니다. p95가 0.2ms에서 2.1ms로 늘어난 것이 그 흔적이고, 이 표의 배수는 애플리케이션의 확장성이 아니라 이 호스트의 천장을 함께 재고 있습니다. 조건마다 1회 실행입니다.
...
2절부터 7절까지는 인프로세스 드라이버로 순차 호출한 것이라 시간축이 없습니다. 8절에서 서버 모드로 띄우고 동시성을 바꿔 가며 초당 접수량을 쟀습니다. 장 시작 트래픽 급증에서의 커넥션풀과 스레드 고갈은 F03에서 부하 테스트로 다룹니다.

해소 쪽에 남아 있던 과제 둘은 2026-07-29 재측정으로 처리했습니다. 유한성 가드의 단독 검증은 조건 B와 D로, 킬스위치의 부수 피해는 조건 C로 쟀습니다. 남은 것은 킬스위치 자체의 값어치입니다. 이 실험은 가드가 완전한 화이트리스트인 조건만 다뤄서, 가드가 놓치는 값이 있을 때 킬스위치가 무엇을 건지는지는 재지 않았습니다. 임계값도 3건 하나만 썼습니다. 실제 운영에서 임계값을 정하려면 초당 주문 수와 거부율의 분포가 필요한데, 8절이 초당 접수량까지는 쟀고 거부율 분포는 여전히 없습니다.
