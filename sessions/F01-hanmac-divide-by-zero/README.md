...
## 가드에 구멍이 있을 때와 임계값 스윕 (2026-07-31)

결과는 `results/spring-killswitch.txt` 입니다.

### 킬스위치가 무엇을 건지는가

앞의 조건들은 가드가 완전한 화이트리스트(`Double.isFinite`)라 NaN 도 Infinity 도 다
잡습니다. 실무의 가드는 자주 불완전합니다. `isInfinite` 만 확인하거나 절댓값으로
상한을 보면 Infinity 는 걸리는데 **NaN 은 두 비교가 모두 false 라 그대로 지나갑니다.**
...
먼저 Infinity 가 섞인 배치(정상 100 + Inf 10 + NaN 10)를 던졌습니다.
...
**두 조건이 같습니다.** 가드가 Infinity 는 잡으므로 그때 킬스위치가 켜지고 뒤따르는
NaN 이 막힙니다. **가드가 못 잡는 값을 킬스위치가 막은 것이 아니라 다른 종류의
비정상이 먼저 와서 대신 켜 준 것입니다.** 방어가 아니라 순서 운입니다.

그 우연을 걷어내려면 가드가 아무것도 못 잡는 배치가 필요합니다. 정상 100 + NaN 20 만
던졌습니다.
...
**E2 는 NaN 20건이 전부 접수됐습니다.** 킬스위치가 세는 것이 "가드가 거부한 건수"라,
가드가 못 잡은 값은 거부되지 않으니 킬스위치도 세지 못합니다. 422 도 503 도 0건입니다.
**아무 일도 일어나지 않은 것처럼 보입니다.**

**F2 는 NaN 접수 0건입니다.** 탐지원을 접수 결과(기준가 이탈)로 옮기면 가드가 무엇을
놓치든 셀 수 있습니다.
...
문턱에 닿지 않습니다. 999 와 결과가 같고, 그것이 곧 가드 단독 조건입니다.

가운데가 교환입니다. 임계값을 낮출수록 비정상은 빨리 멈추고 정상 주문의 부수 피해가
커집니다. **10 에서 정상 52건이 막히고 20 에서 11건이 막힙니다.**
...
앱을 서버 모드로 띄우고 k6 로 동시성을 바꿔 가며 15초씩 걸었습니다. 대상은 7절의 `/orders/leaky` 입니다. 가드에 구멍이 있어 NaN 이 통과하는 엔드포인트이고, 주문의 10%가 `days=0` 이라 NaN 이 됩니다.
...
**차단이 네 조건 모두 0건입니다.** 킬스위치가 한 번도 안 걸립니다. 이 엔드포인트로 들어오는 것은 NaN 인데 킬스위치가 세는 것은 Infinite 뿐이기 때문입니다. 5절이 "탐지원이 가드와 같으면 가드의 구멍이 탐지의 구멍이 된다"고 적은 자리가 시간축 위에서 다시 나옵니다. **접수량이 동시성에 비례해 늘어나는 동안 계측기는 0을 유지합니다.**

동시성을 64배 올렸는데 접수는 11.2배만 늘었습니다. 2 vCPU 컨테이너라 CPU 가 먼저 막힙니다. p95 가 0.2ms 에서 2.1ms 로 늘어난 것이 그 흔적이고, 이 표의 배수는 애플리케이션의 확장성이 아니라 이 호스트의 천장을 함께 재고 있습니다. 조건마다 1회 실행입니다.
