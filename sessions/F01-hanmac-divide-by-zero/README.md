...
버그 엔드포인트 `/orders/buggy`는 정상 100건과 NaN 10건을 201 Created로 접수했습니다. NaN 10건이 실제 REST 검증을 그대로 통과한 것입니다. Infinity 10건만 상한 초과로 422를 받았습니다. 입력 `@Valid` 검증은 120건 모두 통과했습니다. `@Valid`가 보는 것은 클라이언트가 보낸 입력이고, 정작 위험한 값은 서버가 그 뒤에 계산한 파생값이라 검증 범위 밖이기 때문입니다. 코드는 [spring/](spring/)에 있고, 최소 재현과 수치가 정확히 맞물립니다.
