#!/usr/bin/env bash
# A30 공용 헬퍼. 컨테이너는 A24 것을 재사용한다(새로 띄우면 메모리가 모자라다).
# 랩 DB(lostark_log, lostark_ops)는 건드리지 않는다. 이 세션의 DB 는 currency_split 뿐이다.
PW='Lab_Passw0rd!'
DB=currency_split
CT=a24-mssql
SQLCMD=/opt/mssql-tools18/bin/sqlcmd
ACCOUNTS=${ACCOUNTS:-20000}
...
wait_ready(){
  for i in $(seq 1 30); do Q "SELECT 1" | grep -q 1 && return 0; sleep 2; done
