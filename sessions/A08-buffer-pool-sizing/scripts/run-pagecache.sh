...
# fsync면 커널이 한 번 더 캐시한다.
# 어느 쪽이 기본값인지는 버전에 따라 다르다. fsync는 8.0의 기본값이고, 8.4의 Unix 계열
# 기본값은 "O_DIRECT if supported, otherwise fsync"다. 그래서 이 실험이 재는 것은 곁가지가
# 아니라 8.0에서 8.4로 올라갈 때 읽기 경로가 바뀌는 방향이다.
# 그리고 아래 2.6배는 데이터 1424MB가 호스트 페이지 캐시에 통째로 들어가는 크기 관계에서만
# 나온다. 데이터가 페이지 캐시보다 커지면 fsync 쪽 miss도 블록 디바이스로 내려간다.
#
