...
- **반복 측정을 하지 않았습니다.** 조건마다 60초 한 번입니다. 위의 1G 역전처럼 편차인지 실제인지 가리지 못한 대목이 남습니다. 실험 2의 리사이즈 결론도 마찬가지로 조건마다 n이 1입니다. 확대 0.4초, 축소 3.5초, 정지 374.9ms는 전부 한 번씩 관측한 값입니다.
- **콜드에서 출발하지 못했습니다.** `scripts/workload.py`가 임포트 시점에 `COUNT(*)`로 클러스터드 인덱스를 한 번 훑기 때문에, 모든 조건이 풀 스캔 직후에서 워밍업을 시작했습니다. 큰 풀 조건의 0.000페이지는 이 스캔이 만들어 준 상태입니다. 그 줄을 빼거나 `SELECT MIN(id), MAX(id)`만 남기고 다시 재는 것이 남은 과제입니다.
...
- **O_DIRECT 조건의 블록 I/O가 InnoDB가 센 읽기보다 2.0배 큽니다**(약 2,260MiB 대 1,122MiB). 리드어헤드나 파일시스템 계층을 의심했지만 확인하지 못했습니다. 처음에는 2,427MB 대 1,122MB로 2.2배라고 적었는데, `scripts/report.py`가 `docker stats`의 GB에 1024를 곱해 부풀린 값이었습니다. `docker stats`는 SI 접두어를 쓰므로 10^9바이트이고, 2진 MiB로 옮기면 약 2,260입니다. 실험 3의 결론은 `fsync` 쪽이 0이라는 데 기대고 있어서 이 격차와 무관하지만, 숫자를 그대로 남깁니다.
- **버퍼 풀 인스턴스를 1로 고정했습니다.** 8.4는 이 값을 동적으로 계산하므로 논리 프로세서 2개짜리인 이 호스트의 기본값도 8은 아닙니다. 스윕 값이 반올림되지 않게 하려고 고정한 것이라, 인스턴스가 여럿일 때의 경합 특성은 이 측정에 들어 있지 않습니다.
