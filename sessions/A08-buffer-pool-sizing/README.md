...
**기본값을 반대로 알고 있었습니다.** 위 문단을 처음 쓸 때는 `O_DIRECT`가 기본값에서 벗어난 조건이라고 여겼습니다. 반대였습니다. 빠르게 나온 `fsync`가 8.0의 기본값이고, 느리게 나온 `O_DIRECT`가 8.4의 Unix 계열 기본값입니다. 그러니 실험 3이 잰 것은 곁가지가 아니라 8.0에서 8.4로 올라갈 때 읽기 경로가 바뀌는 방향입니다. 이 세션이 잰 것은 두 `flush_method`의 차이지 버전 업그레이드 자체는 아니지만, 두 값이 각각 어느 버전의 기본값인지는 적어 둘 값어치가 있습니다. 스윕 열두 조건이 "기본값을 끄고 돌린 실험"이라던 서술도 이 정정에 따라 사실이 아니게 됩니다.
...
| 조건 | q/s | 히트율 | 디스크 읽기 |
|---|---|---|---|
| warm-128M | 10,883.0 | 82.1165% | 429,028 |
| cold-128M | 10,896.8 | 82.0765% | 430,874 |
| warm-512M | 14,855.7 | 98.9803% | 27,766 |
| cold-512M | 14,872.9 | 98.9777% | 27,870 |
| warm-2G | 15,142.4 | **100.0000%** | **0** |
| cold-2G | 14,733.4 | 99.3331% | **17,945** |

**2G 조건에서만 갈립니다.** 워밍업 전 상태가 히트율 100%와 디스크 읽기 0페이지를
만들고 있었다는 것이 확인됐습니다. 콜드로 출발하면 99.33%에 17,945페이지입니다.
**"큰 풀 조건의 0페이지는 그 스캔이 만들어 준 값"이라고 적어 둔 것이 맞았습니다.**

다만 처리량 영향은 2.7%(15,142 → 14,733)뿐입니다. 워밍업 30초가 대부분을 다시 채우기
...
| **쓰기 20%** | 3.14초 | **79.5ms** | **1,760** |

더티 페이지가 0 근처에서 1,760개로 늘고 최대 지연이 42.8~49.3ms 에서 79.5ms 로
1.6배에서 1.9배 커집니다. **"3.5초는 하한으로 봐야 한다"고 적은 것의 근거가 생겼습니다.**
축소 시간 자체는 3.14초로 읽기 전용 회차와 같은 범위 안에 있으므로, 이 조건에서
늘어난 것은 시간이 아니라 **그동안 조회가 겪는 지연**입니다.
...
| 코어 | 1536M | 2G | 차이 |
|---|---|---|---|
| 2 | 15,019.0 q/s | 15,122.0 q/s | 0.7% |
| 6 | 39,783.1 q/s | 39,625.2 q/s | 0.4% |

코어를 3배로 늘리자 처리량이 2.6배(15,000 → 39,700)로 올랐습니다. **CPU 천장은 확실히
풀렸습니다.** 그런데도 두 크기가 여전히 구별되지 않고, 6코어에서는 1536M 이 오히려
근소하게 높습니다.
...
| inst8-2G | 8 | **8** | 2048MB | 15,510.7 |
| warm-512M | 1 | 1 | 512MB | 14,855.7 |
| inst8-512M | 8 | **1** | 512MB | 15,403.7 |
...
인스턴스를 자동으로 1로 되돌립니다. 그래서 이 줄의 15,403.7 대 14,855.7 은 인스턴스
효과가 아니라 실행 편차입니다.

## 못 한 것

- **축소의 관측이 셋입니다.** 반복 절의 축소는 3.48초와 3.14초와 7.08초이고 그중 하나는 다른 장비입니다. 한 회차는 기록이 실패했습니다. 폭이 2.3배라 중앙값을 말할 표본이 아닙니다.
- **콜드 조건도 워밍업 30초를 그대로 둡니다.** 위 1절은 임포트 시점의 풀 스캔만 없앴고, 측정 전 워밍업은 양쪽 다 있습니다. 워밍업까지 없앤 진짜 콜드 스타트는 재지 않았습니다.
- **쓰기 비율은 20% 하나만 썼습니다.** 위 2절이 더티 1,760개에서 최대 지연 79.5ms 를 보였지만, 비율을 올렸을 때 어디까지 가는지는 재지 않았습니다.
- **1,424MB 보다 큰 데이터로는 재지 않았습니다.** 위 3절에서 1536M 과 2G 가 갈리지 않는 것이 CPU 가 아니라 워킹셋이 다 담겼기 때문임을 확인했습니다(6코어로 늘려 처리량이 2.6배 올랐는데도 두 크기가 같습니다). 데이터를 키워 그 지점을 오른쪽으로 밀면 두 크기가 갈릴 텐데 그 조건은 만들지 않았습니다.
