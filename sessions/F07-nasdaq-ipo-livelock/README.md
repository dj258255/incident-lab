...
10,000ms 조건도 6,440ms 에서 멈췄습니다. **둘 다 시간이 아니라 횟수에 걸렸습니다.**

세 번째 조건에서는 12,000회 상한에 닿기 전 1,517회에 확정됐습니다. 그때 걸린 시간이
5,126ms 입니다. 취소가 한 건도 오지 않는 라운드를 **1,500번 넘게 시도한 끝에 얻은
것**입니다.
...
**시간 상한만 60배로 늘려도 실패가 그대로입니다.** 1초를 60초로 바꿨는데 3/24 이고, 전부 `maxRecalc` 에 걸립니다. 재계산 200회를 못 넘깁니다.

**재계산 상한만 60배로 늘려도 실패가 그대로입니다.** 200을 12,000으로 바꿨는데 역시 3/24 이고, 이번엔 전부 `capMs` 에 걸립니다.
...
그리고 **둘 다 60배로 늘리면 0/24 입니다.** 이 현상은 수렴 자체가 안 되는 것이 아니라 유한하지만 매우 긴 수렴입니다. 본문이 이것을 라이브락이 아니라 기아 또는 수렴하지 않는 재시도 루프라고 부른 것이 이 값으로 뒷받침됩니다. **다만 "유한하다" 는 것은 이 유입률에서의 이야기이고, 유입 간격이 라운드보다 짧은 구간에서는 상한을 얼마로 두든 그 안에 취소가 도착합니다.**
