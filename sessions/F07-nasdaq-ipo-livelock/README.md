...
| 지금 알면 되는 것 | 근거 |
|---|---|
| **블로킹 스레드가 0개인데 일이 안 끝난다.** 데드락도 라이브락도 아닌 재계산 기아다 | 72회 시도 중 **24회** 크로스 확정 실패 ([근거](#2-재현)) |
| **"기록 직전에 근거가 바뀌었으면 다시 계산한다"는 옳게 들리는 규칙에 끝이 없다.** 취소가 라운드보다 촘촘하면 영원히 확정되지 않는다 | ([근거](#3-내부-원리)) |
| **스냅샷 동결과 접수 컷오프로 실패 0/72.** 대가는 반영되지 않은 취소 수십 건이다 | ([근거](#5-재계측)) |
| **상한을 60배로 늘리면 실패가 0 이 되지만, 어느 한쪽만 늘리면 그대로다.** 두 상한이 같은 지점을 가리킨다 | ([근거](#어느-상한이-결정적이었는가)) |
...
정확한 이름은 기아(starvation)이거나 수렴하지 않는 재시도 루프입니다. 계산 스레드에 필요한 자원은 "취소가 한 건도 오지 않는 라운드 하나"인데, 유입이 그 자원을 계속 가져가 버려서 못 얻습니다. 그래도 겉으로 드러나는 증상은 라이브락과 같습니다. 데드락은 스레드 덤프에서 BLOCKED로 보이고 JVM의 교착 탐지에도 걸리지만, 이쪽은 CPU가 바쁘고 계산 스레드가 RUNNABLE이라 "잘 돌고 있는 시스템"처럼 보입니다. 이름을 무엇으로 부르든 밖에서 보이는 그림은 같습니다. 잘 돌고 있는 시스템입니다.
...
이 문서에서 말하는 "확정 실패"의 뜻을 여기서 못박아 둡니다. 영원히 확정되지 않는다는 뜻이 아니라 `compose.yml`이 준 `-DcapMs=1000`에 걸렸다는 뜻입니다. 같은 파일의 `-DmaxRecalc=200`은 세 회차를 통틀어 한 번도 걸리지 않았습니다. 상한을 10초나 60초로 늘렸을 때 어떻게 되는지는 7절에서 따로 쟀습니다. 아래 24회는 "1초 안에 확정하지 못한 횟수"로 읽어야 정확합니다. 그러니 아래 24회는 "1초 안에 확정하지 못한 횟수"로 읽어야 정확합니다.
...
`scripts/run-cap-sweep.sh`, 결과는 `results/cap-sweep.txt` 입니다.

"확정 실패는 영원히 확정되지 않는다는 뜻이 아니라 `-DcapMs` 상한에 걸렸다는 뜻입니다.
상한을 늘렸을 때 결국 확정되는지는 재지 않았습니다"라고 적어 두었습니다. 쟀습니다.
...
**상한을 60배로 늘리니 확정 실패가 0이 됩니다.** 이 현상은 수렴 자체가 안 되는 것이
아니라 **유한하지만 매우 긴 수렴**이었습니다.

### 실패는 전부 한 구간에 몰려 있습니다

취소 유입 간격 여덟 구간 가운데 **2ms 구간 하나에서만** 실패가 납니다.
나머지 일곱 구간은 세 조건 모두 0회입니다.

| `capMs` | 2ms 구간 실패 | 그때 재계산 | 그때 경과 | 실효간격/라운드 |
|---|---|---|---|---|
| 1,000 | 3/3 | **200회** | 755ms | 0.54 |
| 10,000 | 2/3 | **2,000회** | 6,440ms | 0.62 |
| 60,000 | **0/3** | 1,517회 | 5,126ms | 0.59 |

### 걸린 것은 시간 상한이 아니라 횟수 상한이었습니다

앞의 두 조건에서 **재계산 횟수가 `maxRecalc` 값과 정확히 같습니다.** 200과 2,000입니다.
`capMs` 가 1,000ms 인데 경과가 755ms 이므로 시간 상한에는 닿지도 않았습니다.
10,000ms 조건도 6,440ms에서 멈췄습니다. **둘 다 시간이 아니라 횟수에 걸렸습니다.**

세 번째 조건에서는 12,000회 상한에 닿기 전 1,517회에 확정됐습니다. 그때 걸린 시간이
5,126ms입니다. 취소가 한 건도 오지 않는 라운드를 **1,500번 넘게 시도한 끝에 얻은
것**입니다.

### 그러면 이름은 어떻게 되는가

본문은 이것을 라이브락이 아니라 기아 또는 수렴하지 않는 재시도 루프라고 불렀습니다.
그 서술을 한 칸 좁혀야 합니다. **수렴하지 않는 것이 아니라 수렴이 아주 느립니다.**

취소 유입이 고정 타이머라 되먹임 고리가 없다는 지적은 그대로입니다. 그리고 실효간격이
라운드 1회보다 짧은 구간(비율 0.54~0.62)에서만 이 일이 벌어진다는 4절의 결론도
그대로입니다. 달라지는 것은 "영원히"가 아니라 "1,500회 남짓"이라는 크기입니다.

실무로 옮기면 오히려 더 무섭습니다. **재시도 상한을 넉넉히 잡아 두면 언젠가는
확정되지만, 그 사이 5초가 흐릅니다.** 나스닥의 그날은 그 5초가 아니라 19분이었습니다.

### 이 절의 한계

`capMs` 와 `maxRecalc` 를 같은 비율로 함께 올렸습니다. 그래서 세 조건에서 **어느 상한이
결정적이었는지는 앞의 두 조건에서만 갈립니다**(재계산이 상한값과 같으므로 횟수 쪽).
세 번째 조건은 둘 다 안 걸렸습니다. 하나씩만 올리는 조건은 만들지 않았습니다.

조건마다 1회 실행이고, 이 호스트의 버그 구간 실패율 자체가 회차마다 흔들립니다
(발행분은 72회 중 24회, 이 회차의 1,000ms 조건은 24회 중 3회).

## 어느 상한이 결정적이었는가

`capMs` 와 `maxRecalc` 를 같은 비율로 함께 올렸습니다. 그래서 세 조건에서 **어느 상한이 결정적이었는지** 갈리지 않았습니다. 확정 실패마다 어느 쪽에 먼저 닿았는지 기록하게 하고, 한쪽만 올린 조건 둘을 더 넣었습니다.

| `capMs` | `maxRecalc` | 확정 실패 | 닿은 상한 |
|---|---|---|---|
| 1,000 | 200 | 3/24 | **maxRecalc 3건** / capMs 0건 |
| 10,000 | 2,000 | 2/24 | maxRecalc 2건 / capMs 0건 |
| 60,000 | 12,000 | **0/24** | 0건 / 0건 |
| **60,000** | **200** | 3/24 | **maxRecalc 3건** / capMs 0건 |
| **1,000** | **12,000** | 3/24 | maxRecalc 0건 / **capMs 3건** |

**시간 상한만 60배로 늘려도 실패가 그대로입니다.** 1초를 60초로 바꿨는데 3/24이고, 전부 `maxRecalc` 에 걸립니다. 재계산 200회를 못 넘깁니다.

**재계산 상한만 60배로 늘려도 실패가 그대로입니다.** 200을 12,000으로 바꿨는데 역시 3/24이고, 이번엔 전부 `capMs` 에 걸립니다.

**두 상한이 같은 지점을 가리킵니다.** 어느 하나를 풀면 다른 하나가 같은 자리에서 잡습니다. 실패 건수도 3건으로 같습니다. 둘 다 올려야 0이 됩니다.

기본 설정에서 재계산 200회와 1초가 거의 같은 지점인 것은 라운드 1회가 5밀리초 근처이기 때문입니다. 200 × 5밀리초 = 1초입니다. **두 상한을 우연히 같은 곳에 두고 있었습니다.**

그래서 "확정 실패" 라는 판정은 둘 중 하나가 아니라 **둘이 만든 한 경계**입니다. 어느 쪽이 먼저 닿았는지는 조건마다 뒤집히지만 경계 자체는 안 움직입니다.

그리고 **둘 다 60배로 늘리면 0/24입니다.** 이 현상은 수렴 자체가 안 되는 것이 아니라 유한하지만 매우 긴 수렴입니다. 본문이 이것을 라이브락이 아니라 기아 또는 수렴하지 않는 재시도 루프라고 부른 것이 이 값으로 뒷받침됩니다. **다만 "유한하다" 는 것은 이 유입률에서의 이야기이고, 유입 간격이 라운드보다 짧은 구간에서는 상한을 얼마로 두든 그 안에 취소가 도착합니다.**

## 한계

- **다섯 조건은 각각 1회 실행입니다.** 유입 간격 8구간을 3회씩 시도한 값이고, 조건 전체를 반복하지는 않았습니다.
...
"확정 실패"는 1,000ms 상한 안에 확정하지 못했다는 뜻까지만 증명합니다. 상한을 60배로 늘렸을 때 확정되는 것은 7절에서 쟀습니다. 유입이 더 촘촘한 구간에서도 같은 상한이 통하는지는 재지 않았습니다. 기아라는 규정은 되먹임의 부재라는 구조에서 나온 것이지 무한 비확정을 관측해서 나온 것이 아닙니다.
