...
차분 상위의 **계열**이 어느 처방으로 갈지를 정합니다. 정확한 유형은 회차마다 바뀌므로
계열로 봅니다.

| 계열 | 대표 유형 | 블로킹 사슬을 따라가면 | 대신 볼 것 |
|---|---|---|---|
| 락 | `LCK_M_*` | 뿌리 세션이 나온다 | 그 세션이 무엇을 하던 중인지 |
| 메모리·디스크 | `PAGEIOLATCH_*`, `SLEEP_BPOOL_STEAL`, `MEMORY_ALLOCATION_EXT` | **아무도 안 나온다** | 읽는 페이지 수, 버퍼 풀 여유 |
| 로그 | `WRITELOG` | **아무도 안 나온다** | 커밋 횟수. 배치로 묶을 수 있는지 |
| 병렬 | `CXPACKET`, `CXCONSUMER`, `CXSYNC_PORT` | **아무도 안 나온다** | 먼저 **정상인지** 의심한다 |

**락이 아닌데 사슬을 따라가면 빈 결과가 나옵니다.** 그것을 "막는 세션이 없으니 DB
문제가 아니다"로 읽으면 그때부터 헤맵니다.

`SLEEP_BPOOL_STEAL` 이 상위면 **디스크가 아니라 메모리** 문제입니다. 같은 페이지를
반복해 올리고 있다는 뜻이라 디스크를 바꿔도 안 낫습니다.

---

## 사고가 지나간 뒤라면

DMV 는 **지금**을 보는 도구입니다. 사고가 끝났으면 아래 둘을 봅니다.

| 무엇을 | 어디서 |
|---|---|
| 그 구간의 대기 | 수집기가 떠 둔 스냅샷의 **구간 차분** |
| 그 구간의 쿼리와 계획 | **Query Store** |
| 지금 누가 막고 있는지 | DMV 뿐. 둘 다 못 한다 |

`dm_exec_query_stats` 는 **캐시에 있는 계획의** 통계입니다. `DBCC FREEPROCCACHE`
한 번에 3계획·187,800 논리 읽기가 0 이 됐고 Query Store 는 그대로였습니다.

```sql
-- 대기 수집기. 누적값을 그대로 담고 차분은 읽을 때 낸다.
CREATE OR ALTER PROCEDURE usp_CollectWaits @tick INT AS
BEGIN
    SET NOCOUNT ON;
    INSERT INTO diag_wait_snap (tick, wait_type, wait_ms, tasks)
    SELECT @tick, wait_type, wait_time_ms, waiting_tasks_count
      FROM sys.dm_os_wait_stats
     WHERE wait_type NOT IN (<유휴 대기 목록>) AND wait_time_ms > 0;
END
```

| 정한 것 | 이유 |
|---|---|
| 누적값을 그대로 담는다 | 수집할 때 차분을 내면 구간을 다시 못 고르고 재시작도 못 가른다 |
| 유휴 대기는 **담을 때** 뺀다 | 담아 두면 표가 커지고 매번 같은 것을 걸러야 한다 |
| 보존 기간을 정한다 | 1분 주기로 1년이면 4.6GB, 30일이면 380MB |
| 무시 목록은 **한 곳**에 둔다 | 따로 만들면 빠뜨린다. 실측에서 두 번 그랬다 |

Query Store 를 켤 때는 **캡처 모드를 `ALL`** 로 둡니다. 기본값 `AUTO` 는 가벼운 쿼리를
안 담는데, 리터럴로 흩어진 범인은 하나하나가 가볍습니다.
