...
>
> **발행** — [느리다는 신고는 왔는데 느린 쿼리 목록은 멀쩡하다](https://dj258255.github.io/IT-Oasis/blog/incident/mssql-diagnosis) (시리즈 3편)

## 결론부터

| 지금 알면 되는 것 | 근거 |
|---|---|
| **대기 통계는 인스턴스 기동 이후 누적이라 지금을 못 본다.** 두 번 떠서 차분으로 봐야 한다 | 같은 `LCK_M_IS` 가 누적 430,521ms 대 이번 사고 107,453ms ([근거](#3-대기-통계는-누적이라-지금을-못-본다)) |
| **유휴 대기를 무시 목록에서 빠뜨리면 차분을 떠도 신호가 묻힌다** | `SOS_WORK_DISPATCHER` 등 ([근거](#3-대기-통계는-누적이라-지금을-못-본다)) |
| **블로킹의 뿌리는 sleeping 이라 `dm_exec_requests` 에 안 나온다.** 요청은 없는데 트랜잭션만 열린 세션으로 찾아야 한다 | 재귀로 사슬 53←52←51 ([근거](#4-막은-세션이-아니라-뿌리를-찾는다)) |
| **계획 캐시를 계획 단위로 정렬하면 리터럴 쿼리가 60개로 흩어져 안 잡힌다.** `query_hash` 로 묶어야 실제 부하가 드러난다 | 151,326 → 304,086 ([근거](#5-순위에-안-잡히는-범인)) |
| **대기가 락이 아니면 처방이 통째로 안 맞는다.** 그래서 순서가 대기 → 뿌리 → 쿼리다 | ([근거](#6-대기가-락이-아니면-처방이-통째로-안-맞는다)) |
...
**순서가 없으면 무엇을 놓치는가에 대한 공개 사례.** Roblox 는 2021년 10월 73시간 장애에서
네 번 오진했고 **네 번 모두 "자원을 더 넣는" 방향**이었다(하드웨어 교체 13시간, 용량 증설 24시간,
서비스 축소 22시간, 코어 스케일링 12시간). 실제 원인은 자원이 아니라 **직렬화 지점의 경합**이었고,
결정적 단서는 54시간이 지나 프로파일링에서 나왔다. 게다가 **"장애 원인을 볼 수 있게 해 줄 모니터링
시스템이 영향받은 시스템에 의존"** 해 진단 자체가 가려졌다. 이 세션이 대기 → 뿌리 → 쿼리 순서를
고정한 이유이고, 수집기를 관리 대상과 분리해 둔 이유이기도 하다.
