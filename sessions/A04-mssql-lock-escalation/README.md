...
- **시간을 재지 않았습니다.** ARM 에뮬레이션이라 배치 분할이 한 방보다 총 얼마나
  느린지는 이 랩에서 답할 수 없습니다. **랩 조건 때문에 못 닫은 항목입니다.**
- **재시도 간격이 정확히 1,250 인지 못 밝혔습니다.** 실패한 승격 시도는 확장 이벤트에
  안 남습니다. 두 각도로 시도했는데 하나는 문장 단위 판정 때문에 성립하지 않았고
  (그 대신 13절의 발견을 얻었습니다), 하나는 표본 간격 0.3초 안에 승격이 끝나 못
  잡았습니다. **간격을 좁히려면 엔진 내부를 보는 다른 도구가 필요합니다.**
- **경계가 회차마다 한 칸 움직이는 것을 재현 못 했습니다.** 5회 모두 같은 값이었습니다.
  통계를 새로 만든 뒤에 재기 때문으로 보이지만 확인은 없습니다.
- **락 메모리 압박을 못 만들었습니다.** 행을 늘리면 엔진이 페이지 락으로 갈아타
  락이 오히려 줄어듭니다. 행 락을 강제하면서 압박까지 가는 길은 안 찾았습니다.
- **지터가 고정 지연보다 나은 조건을 못 만들었습니다.** 세션 2개와 20개에서 두 번
  쟀고 두 번 다 안 갈렸습니다. 승격이 나면 줄을 서고 안 나면 안 부딪힙니다.
