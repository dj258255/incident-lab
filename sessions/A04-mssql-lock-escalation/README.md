...
1. **대량 갱신은 5,000행 미만 배치로 쪼갠다.** 6,250이라는 여유를 알아도 5,000을
   기준으로 잡습니다. 이 여유는 문서가 보장하는 값이 아니고 빌드에 따라 달라질 수
   있습니다.
...
- **시간을 재지 않았습니다.** ARM 에뮬레이션이라 절대값이 이 호스트의 것이 아닙니다.
  ...
  중요한 값인데 비어 있습니다.
- **인덱스가 하나뿐입니다.** 클러스터드 PK 만 있는 테이블이라 보조 인덱스에 붙는
  락이 임계값 계산에 어떻게 들어가는지 보지 못했습니다. 실제 재화 테이블은 계정과
  재화 종류에 인덱스가 더 붙습니다.
