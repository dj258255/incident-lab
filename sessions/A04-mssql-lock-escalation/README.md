...
- **재시도가 실제로 1,250개마다 일어나는지는 못 봤습니다.** 실험 7에서 승격이 실패하는
  것까지는 봤지만, 옆 세션이 버티는 동안 몇 번을 다시 시도했는지는 확장 이벤트에
  안 남습니다. 실패한 시도는 이벤트를 안 남기기 때문입니다.
- **지터가 고정 지연보다 나은 조건을 안 만들었습니다.** 실험 11에서 둘이 같게
  나왔는데, 세션이 둘뿐이고 트랜잭션이 3초를 기다려 충돌 창이 넓었기 때문입니다.
  경쟁자를 수십 개로 늘려야 갈릴 텐데 그 규모는 안 돌렸습니다.
- **재시도 상한에 닿는 경우를 못 봤습니다.** 5회 상한을 뒀는데 최대 2회에서 끝나
  "상한에 닿으면 멈추고 사람을 부른다"는 절차가 실제로 동작하는지는 확인 못 했습니다.
...
- **hobt_lock_count 가 정확히 5,000 이 아니라 5,003~5,007 입니다.** 검사 지점 사이에
  여러 락이 한 번에 붙어서로 보이지만, 그 몇 개의 차이가 어디서 오는지는 안 팠습니다.
