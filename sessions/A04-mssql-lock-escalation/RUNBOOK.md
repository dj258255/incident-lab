...
승격은 **문장 하나가 잡은 락**으로 판정됩니다. 트랜잭션이 든 락이 아닙니다.

| 같은 트랜잭션에서 | 총 락 수 | 승격 |
|---|---|---|
| 300행씩 60문장 | 18,050 | **안 남** |
| 18,000행 한 문장 | — | 남 |

**그래서 배치 분할이 통합니다.** 배치마다 커밋하지 않고 한 트랜잭션에 묶어 두어도
승격은 안 나지만, 락이 그대로 쌓이므로 커밋해서 놓아 주는 것이 맞습니다.

경계는 갱신 컬럼에 붙은 보조 인덱스 수가 정합니다.
...
**하나만 붙어도 경계가 절반 아래로 떨어지고, 그 뒤로는 2,500 근처에서 평평합니다.**
승격이 인덱스 하나의 락으로 판정되기 때문입니다. 다만 **경계는 그대로인데 총 락 수는
계속 늘어나** 락 매니저 부담이 커집니다.

> 문서의 5,000 은 **발동 지점이 아니라 하한**입니다. 엔진은 문장 락 1,250개마다
> 검사하므로 실제 발동은 5,000 과 5,000+검사간격 사이입니다. 실측에서 초과분이
> 인덱스 구성에 따라 0~1,248 로 벌어졌고 `escalated_lock_count` 는 항상 1,250 배수
> 바로 아래였습니다. **경계를 5,000 으로 계산하면 매번 어긋납니다.**
...
마지막 줄이 실무의 기본값입니다. 그 표에 인덱스가 몇 개인지는 절차서를 쓰는 사람이
모를 수 있으므로, **프로시저가 청크 상한을 강제**하는 편이 안전합니다(A25).

`LOCK_ESCALATION = DISABLE` 은 20만 락까지 안전한 것을 확인했지만 **"절대 안 난다"가
아닙니다.** 메모리 압박에서는 예외이고 이 랩은 그 압박을 못 만들었습니다.
행을 늘리면 엔진이 페이지 락으로 갈아타 락이 오히려 줄어듭니다.

---

## DISABLE 은 어디까지 버티는가

`LOCK_ESCALATION = DISABLE` 로 200만 행을 갱신하며 락을 쌓았습니다.

| 갱신 행 | 힌트 | 총 락 | KEY | 락 매니저 메모리 | 승격 |
|---|---|---|---|---|---|
| 100만 | 없음 | 2,719 | 0 | 2MB | 없음 |
| 100만 | `ROWLOCK` | 1,002,719 | 1,000,000 | 188MB | 없음 |
| 200만 | `ROWLOCK` | 2,005,436 | 2,000,000 | **555MB** | 없음 |

**힌트가 없으면 엔진이 처음부터 페이지 락을 고릅니다.** 락을 실제로 쌓으려면
`WITH (ROWLOCK)` 이 필요하고, 그렇게 쌓아도 `DISABLE` 이면 승격은 안 났습니다.

그러나 **"`DISABLE` 이면 안전하다"로 읽으면 안 됩니다.** 200만 락에 락 매니저가
555MB 를 씁니다. 승격이 안 나는 대신 **그 메모리를 계속 들고 있고** 그 행들을 끝까지
붙잡습니다. 문서는 메모리 압박에서 예외라고 적고, 이 랩은 그 압박까지는 못 갔습니다.

| 상황 | 판단 |
|---|---|
| 보정 배치를 쪼갠다 | **기본.** 승격이 문장 단위 판정이라 이것으로 충분하다 |
| `DISABLE` 을 켠다 | 락 메모리를 대신 낸다. 배치를 안 쪼개도 된다는 뜻이 아니다 |
| `ROWLOCK` 힌트를 준다 | 엔진의 굵은 단위 선택을 막는다. 락이 자릿수로 는다 |
