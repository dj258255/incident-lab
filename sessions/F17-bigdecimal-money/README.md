...
| 지금 알면 되는 것 | 근거 |
|---|---|
| **절삭은 한 방향으로만 깎인다.** 밴쿠버 1982 방식으로 60,000회 갱신하면 지수가 29.9포인트 내려앉는다 | 기대 손실 30포인트와 일치 ([근거](#2-재현)) |
| **`HALF_EVEN` 으로 바꾸면 같은 60,000회에서 오차가 1,511분의 1 이 된다** | 29.919802 → 0.019802 ([근거](#5-재계측)) |
| **`BigDecimal` 과 최소 화폐단위 `long` 은 100만 건 누적에서 오차 0** | ([근거](#5-재계측)) |
| **`scale` 과 `RoundingMode` 를 명시하지 않으면 이 오차가 조용히 쌓인다** | ([근거](#4-해소)) |
