...
| 지금 알면 되는 것 | 근거 |
|---|---|
| **원인은 vacuum 을 끈 것이 아니었다.** `autovacuum=off` 여도 wraparound 방지 vacuum 은 돈다. 동결을 막은 것은 **버려진 prepared transaction** 이다 | ([근거](#2-재현)) |
| **정지 지점은 100만이 아니라 300만이다.** Sentry 가 인용한 100만은 PG13 이하 값 | 경고 4,000만 / 정지 300만 ([근거](#2-재현)) |
| **정지 상태에서도 SELECT 와 VACUUM 은 된다.** 거부되는 것은 XID 를 새로 따는 일이다 | `txid_current()` 거부 ([근거](#3-재계측)) |
| **원인을 없애면 40초 만에 자가 복구된다.** 긴급 autovacuum 이 template0 까지 돈다 | PG14 `vacuum_failsafe_age` 발동 로그 ([근거](#5-vacuum_failsafe_age가-아끼는-것과-대신-내는-것)) |
| **서브트랜잭션은 XID 를 정확히 n+1 개 쓴다** | ([근거](#서브트랜잭션이-xid를-얼마나-더-쓰는가)) |
