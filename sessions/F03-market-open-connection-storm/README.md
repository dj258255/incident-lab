...
필요량은 계산으로도 맞습니다. 초당 333건에 응답 54ms 이므로 동시에 처리 중인 요청이 평균 `333 × 0.054 = 18` 개입니다. 풀 20이 거의 맞아 615건만 버리고, 10은 절반이 모자라고, 50 이상은 남습니다.

### max-threads 를 반복해서

기존 값은 조건마다 1회였습니다. 3회씩 다시 쟀습니다. 버그 경로입니다.

| max-threads | p95 중앙 | p95 최소 | p95 최대 | 초당 중앙 |
|---|---|---|---|---|
| 10 | 14.0ms | 13.9ms | 14.0ms | 182.0 |
| 50 | 13.7ms | 13.7ms | 13.7ms | 187.6 |
| 200 | 13.3ms | 13.3ms | 13.3ms | 200.8 |
| 800 | **2.0ms** | 2.0ms | 2.0ms | **324.7** |

**회차 폭이 0.1ms 안입니다.** 조건 사이의 차이는 11.3ms 이므로 방향을 믿을 수 있습니다. 앞서 1회 값으로 적은 단조 증가가 반복 측정에서도 그대로입니다.

10에서 200까지는 완만합니다. p95 가 14.0에서 13.3ms 로 5% 좋아지고 처리량이 182에서 201로 10% 늘어납니다. **800에서 계단이 있습니다.** p95 가 2.0ms 로 6.7배 좋아지고 처리량이 324.7 로 1.6배 늘어납니다.

버그 경로에서 스레드를 늘리는 것이 이만큼 듣는 것은, 이 경로의 대기가 스레드를 잡은 채로 쌓이기 때문입니다. 스레드가 800이면 쌓일 자리가 충분해 큐가 앞단에 안 생깁니다. **같은 부하에서 풀은 아무것도 안 바꾸고 스레드 수는 6.7배를 바꿉니다.** 병목이 있는 층을 먼저 찾아야 하는 이유입니다.
