...
**"못 탈 때"가 전부 풀 테이블 스캔은 아닙니다.** `type: ALL`은 형변환 하나뿐입니다. 함수 적용과 선행 와일드카드는 `Extra: Using index`가 붙은 커버링 인덱스 스캔이라, 테이블의 데이터 페이지가 아니라 인덱스 리프를 처음부터 끝까지 훑습니다. 이 두 줄의 300만 행을 "테이블을 300만 번 읽었다"로 읽으면 안 됩니다.
