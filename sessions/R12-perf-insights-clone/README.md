...
핫 로우 UPDATE 458 샘플이 전부 `cpu` 입니다. 락 경합이 있는 워크로드인데 대기가 한 건도 안 잡힙니다. 처음에는 이것을 InnoDB 행 락이 `events_waits_current` 계층에 안 나타나기 때문이라고 적었는데, **틀렸습니다.** 원인은 이 세션이 앞에서 이미 문서화한 함정입니다. `setup_consumers` 의 `%waits%` 셋이 그 실행에서 꺼져 있었습니다.
