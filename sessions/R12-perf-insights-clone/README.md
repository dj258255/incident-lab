...
### 대기 분해가 전부 cpu 인 것이 이 세션의 한계를 다시 보인다

핫 로우 UPDATE 458 샘플이 전부 `cpu` 입니다. 락 경합이 있는 워크로드인데
`wait/lock` 으로 잡히지 않습니다. 원 세션이 "락인지 진짜 IO 인지 갈리지 않는다"고 적은
것과 같은 자리입니다. `events_waits_current` 는 그 순간 **대기 중인** 이벤트만 담고,
InnoDB 행 락 대기는 그 계층에 나타나지 않습니다. `data_lock_waits` 를 따로 세야 갈립니다
(`lock-probe.py` 가 그 자리입니다).
...
**앞의 둘은 같은 값을 줍니다.** 21과 28로 완전히 같습니다. `sys` 뷰가 `data_lock_waits` 를
포함한 여러 테이블을 조인한 것이니 당연합니다. 갈리는 것은 비용이고, **2.6배 비쌉니다.**
...
비용이 비슷해 보이지만(0.75ms) 형태가 다릅니다. 텍스트가 중앙 10,327바이트이고 매번
...
부하가 있을 때의 비용 순위는 두 회차가 같습니다. 앞 표가 0.68 / 1.77 / 0.75ms 였고 이번이 0.69 / 1.83 / 0.79ms 입니다. **`sys` 뷰가 가장 비싸다는 결론은 회차를 늘려도 그대로입니다.**
