...
|---|---|---|---|
| `old_blocks_time=0` | 22회 | 129,216 | **2,842,752페이지** |
| `old_blocks_time=1000` | 25회 | 129,216 | **3,230,400페이지** |
...
| `mysqldump` | 0 (방어 끔) | **2회** | 4,681MB | 2.82ms | 3.17ms | **54.1%** | 5.5초 |
| `mysqldump` | 1000 (기본) | **2회** | 4,681MB | 2.72ms | 3.08ms | **47.2%** | 0.6초 |

**`mysqldump` 가 집계 스캔보다 버퍼 풀을 덜 오염시킵니다.** 히트율 최저점이 47~54% 인데 집계는 5~9% 입니다. 출력 I/O 가 겹쳐 더 나쁠 것으로 봤는데 반대입니다.

이유는 속도입니다. **60초 동안 집계는 22~25회 도는데 `mysqldump` 는 2회입니다.** 덤프는 행을 `INSERT` 문자열로 직렬화하고 4.68GB 를 밖으로 내보냅니다. 그 일이 읽기보다 오래 걸려서 **초당 훑는 페이지가 11배 적습니다.** 오염은 초당 몇 페이지를 밀어내느냐로 정해지므로 그만큼 덜 밀어냅니다.
...
원 사례와의 거리도 그대로입니다. Percona 가 본 150배 붕괴는 `mysqldump` 조건에서도 안 옵니다. 그 실험의 회전 디스크와 이 호스트의 NVMe 차이이고, 배치를 원 사례와 같은 도구로 바꿔도 그 축은 안 움직입니다.
