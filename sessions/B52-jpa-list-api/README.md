...
- **커서의 타이브레이커 분기를 한 번도 밟지 않았습니다.** `seed.py`가 `created_at`을 3초 간격으로 넣어 방송 20만 건이 전부 서로 다른 시각입니다. 그래서 풀어쓴 조건의 `created_at = ? AND id > ?` 쪽은 늘 거짓이었고, 실질적으로 `created_at > ?` 하나만 잰 셈입니다. 같은 시각이 몰리는 데이터에서 두 문법의 차이가 그대로인지는 재지 않았습니다.
