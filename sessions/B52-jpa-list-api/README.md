...
가장 널리 인용되는 커서 문법이 MySQL에서는 듣지 않습니다. 커서 페이지네이션을 도입하고 "왜 그대로지"라고 하는 상황의 정확한 원인입니다. `EXPLAIN`의 `type` 컬럼이 `range`인지 `index`인지만 확인하면 즉시 드러납니다.
...
이 세션의 핵심 결론은 행값 비교가 MySQL 8.4.3에서 범위 최적화를 받지 못한다는 것이었습니다.
PostgreSQL은 정반대입니다.
...
MySQL에서 옳은 처방을 PostgreSQL에 그대로 옮기면 117배 손해입니다.
...
`saveAll` 직접 ID가 빈 `sponsor_assigned` 에 넣어 출발선이 달랐습니다.
...
**시간은 회차마다 다릅니다.** run0이 모든 조건에서 가장 느리고 나머지 셋은 붙어 있습니다.
...
**행당 쿼리는 계속 내려가는데 시간은 100에서 멈춥니다.** 100에서 5000까지 행당 쿼리가
1.021에서 1.001로 줄지만 소요는 8.81초에서 8.79초로 같습니다. 남은 0.02건이 아끼는
...
7절이 쓴 500은 이득이 이미 다 나온 구간입니다. **100으로 낮춰도 같고, 5000으로 올려도
...
이어 붙는 조건이라 삽입이 항상 오른쪽 끝에서 일어나고, B-Tree가 깊어져도 그 경로만
...
**시간은 거의 같고 공간이 26배입니다.** 0.20초와 0.23초는 15% 차이이고, 1.0MB와 26.0MB는 26배입니다.

**삽입 시간만 보면 이 대가가 안 보입니다.** 200만 행이 버퍼 풀에 다 들어가 있어 페이지 분할이 디스크 읽기를 안 일으킵니다. 메모리 안에서 페이지를 쪼개고 반씩 채우는 것뿐이라 시간이 거의 안 듭니다.

**대가는 나중에 옵니다.** 26MB가 더 늘었다는 것은 같은 행 수가 더 많은 페이지에 흩어졌다는 뜻입니다. 그 표를 읽을 때 버퍼 풀에 올려야 할 페이지가 그만큼 늘고, 백업도 그만큼 커지고, 복제도 그만큼 더 보냅니다. 9절의 "같은 비용" 은 **삽입 시점의 시간에 한해서** 맞습니다.
...
- **순증 대 무작위는 SQL로 직접 쟀습니다.** JPA 경로를 통과시키지는 않았습니다. 메커니즘이 DB 쪽이라 결과는 같겠지만 배치 크기와의 상호작용은 안 봤습니다.
- **초기 회차의 호스트 사양이 없습니다.** 8절과 9절은 macOS 26.3.1, Apple M2 Pro 12코어, 32GB에서 돌았고 그 이전 회차의 장비는 기록이 없습니다.
- **대량 삽입에 워밍업이 없습니다.** 8절에서 회차를 넷으로 늘렸지만 회차마다 POST는 여전히 한 번이고, run0이 모든 조건에서 가장 느린 것도 그 때문일 수 있습니다.
- **무작위 `id` 로는 재지 않았습니다.** 9절에서 200만 행이 든 테이블도 같은 비용임을 확인했지만, 그것은 `id` 가 순증하는 조건입니다. UUID처럼 흩어지는 키는 A17이 다룹니다.
...
- **PostgreSQL 쪽은 페이지네이션만 대조했습니다.** 6절은 커서 문법 하나이고, N+1과 대량 삽입은 PostgreSQL에서 재지 않았습니다. `reWriteBatchedInserts` 도 재지 않았습니다.
