...
데이터는 Clerk의 조건을 축소한 것입니다. `req_log`는 1,200만 행(507MB, 64,868페이지)이고 `blocked_until` 컬럼은 요청이 레이트리밋에 걸렸을 때만 값이 찹니다. non-null은 2,400행으로 NULL 비율이 99.98%입니다. non-null 행을 마지막에 넣어 디스크상 테이블 끝쪽에 몰리게 했습니다. 그 2,400행이 실제로 몇 페이지에 들어갔는지는 뒤에 나오는 비트맵 스캔이 알려 줍니다. `Heap Blocks: exact=17`, 즉 64,868페이지 중 17페이지입니다. 이 배치가 3절에서 다시 나옵니다. `ip_rule`은 조직별 IP 차단 규칙 200만 행이고 org_id에만 인덱스가 있습니다. 조직 100개에 조직마다 20,000행이 붙어 있고 그중 `rule_kind='burst'`는 조직마다 1행뿐입니다. `rule_kind`가 인덱스에 없다는 이 조건도 뒤에서 배수의 크기를 정하는 자리에 다시 나옵니다.
...
## null_frac=1 의 확률, 컬럼 하나만 올리기, 자동 analyze (2026-07-31)
...
`ANALYZE` 의 표본 크기는 컬럼별이 아니라 테이블 단위로 `300 × (대상 컬럼 target 의
최댓값)` 입니다. 그러면 한 컬럼만 올려도 표본은 같아집니다.

| 조건 | ANALYZE | 저장된 값 |
|---|---|---|
| 네 컬럼 모두 target=100 | 0.23초 | 205 |
| blocked_until 만 target=100 | 0.21초 | 209 |
| 네 컬럼 모두 target=1000 | 0.74초 | **1,152** |
| blocked_until 만 target=1000 | 0.71초 | **275** |

**`ANALYZE` 소요는 두 조건이 같습니다.** target=1000 에서 0.74초와 0.71초입니다.
표본 크기가 같으니 스캔 비용도 같습니다.

**저장되는 값의 개수만 갈립니다.** 1,152개와 275개로 4.2배입니다. 네 컬럼을 함께
올리면 컬럼마다 MCV 와 히스토그램을 최대 1,000개씩 저장하고, 한 컬럼만 올리면 그
컬럼만 저장합니다. **컬럼을 고르는 것으로 아끼는 것은 스캔이 아니라 통계의 양과
그것을 읽는 플래너의 시간입니다.**

### 자동 analyze 는 관측 창 안에 안 걸렸습니다
...
## target 이 플래너 시간을 얼마나 늘리는가 (2026-07-31)

2절은 "컬럼을 고르는 것으로 아끼는 것은 스캔이 아니라 통계의 양과 그것을 읽는 플래너의 시간" 이라고 적었습니다. 플래너 시간은 안 쟀습니다. `EXPLAIN` 의 `Planning Time` 을 target 다섯 값에서 9회씩 재고 중앙값을 잡았습니다.
...
**통계를 231배 늘려도 플래너 시간이 안 늘어납니다.** target 1 에서 1000 으로 가면 저장된 값이 5개에서 1,156개가 되는데 플래닝은 0.607ms 에서 0.616ms 입니다. 회차 폭 안입니다.

**target 10000 에서만 움직입니다.** 0.724ms 로 19% 늘어납니다. 저장된 값이 10,723개일 때입니다.
...
`pg_statistic` 크기도 target 1000 까지 311,296B 로 같습니다. 값이 231배 늘었는데 파일 크기가 안 변합니다. 저장이 페이지 단위이고 배열이 TOAST 되기 때문입니다. **크기로 통계의 양을 재면 안 됩니다.**

**"target 을 올리면 플래닝이 느려진다" 는 이 규모에서 안 맞습니다.** 2절에 적은 "그것을 읽는 플래너의 시간" 이라는 표현은 근거 없이 쓴 것이었고, 실측은 target 1000 까지 차이가 없습니다.

다만 절대값을 봐야 합니다. 플래닝 0.6ms 는 질의 하나입니다. 이 질의가 초당 5,000건 돈다면 플래닝만으로 코어 3개를 씁니다. **target 을 올려서 늘어나는 것이 아니라 원래 그만큼 든다는 것이 이 표가 말하는 쪽입니다.** 준비된 문장(prepared statement)이나 계획 캐시가 필요한 이유가 여기 있습니다.

## 한계

- **플래너 시간은 질의 하나로만 쟀습니다.** 조인 두 개짜리 질의 하나이고, 조인이 많은 질의에서 target 이 어떻게 붙는지는 안 봤습니다. 조인 순서 탐색은 통계와 별개로 테이블 수에 지수로 붙습니다.
