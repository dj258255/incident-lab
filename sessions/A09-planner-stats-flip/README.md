...
| 지금 알면 되는 것 | 근거 |
|---|---|
| **표본 300행이 1,200만 행을 100% NULL 이라고 단정한다.** `null_frac` 이 1로 적히면 추정이 1행이 되고 플래너가 중첩 루프를 고른다 | 추정 1행 대 실제 2,400행, **7,734ms** ([근거](#2-재현)) |
| **statistics target 1000 으로 47배.** 버퍼는 27분의 1 | 7,734ms → **164ms** ([근거](#5-재계측)) |
| **기본값 100 에서도 10회 중 1회 오판한다.** 원인은 블록 뭉침이다. 2,400행이 단 17블록에 몰려 있어 표본이 통째로 빗나간다 | ([근거](#기본값에서-왜-자주-나는가--블록-뭉침이-맞았습니다)) |
| **"target 을 올리면 플래닝이 느려진다"가 이 규모에서는 안 맞는다.** 통계를 231배 늘려도 플래너 시간이 안 늘고 10000 에서만 움직인다 | ([근거](#target이-플래너-시간을-얼마나-늘리는가)) |
...
## null_frac=1의 확률, 컬럼 하나만 올리기, 자동 analyze
...
**원인은 블록 뭉침이 맞았습니다.** 같은 행 수·같은 non-null 로 뭉친 것과 흩은 것을
300회씩 대조해 확인했습니다(집중형 5/300 대 분산형 1/300). 아래
「기본값에서 왜 자주 나는가」 절입니다.

> **처음에는 반대로 적었습니다(2026-08-14 정정).** `target=100` 이면 64,868블록 중
> 30,000개를 읽으니 17개 블록을 전부 놓칠 확률이 `(1-30000/64868)^17` 약 0.0034%,
> 단순 무작위보다 오히려 작다 — 그래서 "뭉침은 반대 방향으로 작용한다"고 썼습니다.
> **1단계만 센 계산이었습니다.** 블록이 뽑혀도 그 안의 non-null 이 저수지 표본에서
> 탈락하는 2단계가 빠져 있었습니다. 2단계를 넣으면 방향이 뒤집힙니다.
...
## 기본값에서 왜 자주 나는가 — 블록 뭉침이 맞았습니다

[scripts/exp17-block-clustering.sh](scripts/exp17-block-clustering.sh) · [results/exp17-block-clustering.txt](results/exp17-block-clustering.txt)

두 가지를 바꿔서 다시 쟀습니다.

**희귀 사건을 세지 않았습니다.** `null_frac=1` 은 1%대 사건이라 10회로는 아무것도
못 정합니다. 대신 매 회 **표본에 들어온 non-null 개수**를 기록했습니다.
`null_frac` 하나로 복원됩니다(`30000 × (1 - null_frac)`). 회당 얻는 정보가 훨씬
많아 300회로 모형이 갈립니다.

**대조군을 만들었습니다.** 같은 1,200만 행·같은 2,400건인데 물리적으로 흩뿌린 표를
하나 더 만들었습니다. non-null 이 든 블록이 **17개 대 2,400개**입니다. 다른 것은
없습니다.

| 조건 | 표본 non-null 평균 | 분산 | 표본에 0건 |
|---|---|---|---|
| 집중형 (17블록) | 5.87 | **9.94** | 5/300 = 1.67% |
| 분산형 (2,400블록) | 5.69 | **5.58** | 1/300 = 0.33% |

분산비 `F = 1.782` (df 299,299), `p = 3.6e-07`.

**평균은 같고 분산만 갈립니다.** 같은 2,400건이니 평균이 같은 것은 당연합니다.
몰려 있으면 "여러 건이 한꺼번에 들어오거나 한꺼번에 빠지거나"가 되어 흔들림이
커지고, **그 꼬리가 '한 건도 못 잡음'입니다.**

이론값과도 맞습니다. 분산형은 예측 5.98 / 실측 5.58 로, 블록 단계가 사실상 무력해져
단순 무작위 표본이 됩니다. 집중형은 예측 8.43 / 실측 9.94 로 조금 더 흔들리는데,
모형이 17블록에 141건씩 고르게 들었다고 둔 반면 실제로는 끝 블록이 덜 찼습니다.

**근거는 소스 주석입니다.** `src/backend/commands/analyze.c` 가 두 단계를 적어 두고
한계까지 인정합니다.

> Stage one selects up to targrows random blocks (or all blocks, if there aren't
> so many). Stage two scans these blocks and uses the Vitter algorithm to create
...
**"40배"는 시행 부족이 만든 값이었습니다.** 집중형의 실제 비율이 1.67% 이므로 10회에서
1회 이상 볼 확률은 15.5% 입니다. 드물지 않습니다. 10회 시행의 신뢰구간은
[0.25%, 45%] 라 어떤 모형도 기각하지 못했습니다. 뭉침이 실제로 만드는 몫은 **약 5배**
(1.67% 대 0.33%)입니다. 방향은 맞았고 크기가 틀렸습니다.

**그리고 이 모양은 인위적이지 않습니다.** 시드가 차단 요청 2,400건을 표 맨 뒤에
붙여서 집중형이 나왔는데, 실서비스의 차단 로그도 버스트로 몰려 들어오므로
append-only 로그에서는 이쪽이 오히려 흔한 모양입니다. `CLUSTER` 나 재적재로 물리
순서가 바뀌면 같은 데이터가 분산형이 됩니다.
