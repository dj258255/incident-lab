...
**`terminate` 는 넷까지 견딥니다.** 수신 합이 670,224에서 667,666으로 0.4% 줄어듭니다. 회차 하나짜리 값이므로 이 정도 차이는 편차와 구별되지 않습니다. max 는 10ms 에서 27ms 로 늡니다. 넘친 구독자만 끊고 나머지는 그대로 흐르므로, 절단은 느린 구독자 수에 대해 무너지지 않습니다.
