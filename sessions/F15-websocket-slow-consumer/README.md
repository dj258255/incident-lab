...
| conflation | 2,907 | 46.7MB | 0.01MB | 0.3초 | 0.2초 | 1ms | 0 | 끊지 않음 |
| 절단 1MB/3초 | 2,949 | 50.7MB | 3.70MB | 0.1초 | 0.2초 | 1ms | 0 | 6.9초에 절단 |
...
느린 구독자 하나의 문제라고 생각했는데 정상 구독자가 같이 죽었습니다. 느린 구독자 하나의 문제라고 생각했는데, 직접 전송 조건에서 정상 구독자 5명이 받은 마지막 시퀀스가 3,265번이었습니다. 대조군은 129,138번입니다. 느린 사람 한 명이 나머지 전원의 데이터를 40분의 1로 줄였습니다.

덜 해로운 쪽은 뜻밖에도 아무것도 안 읽는 구독자였습니다. 초당 32KB만 읽는 구독자를 붙였을 때 발행량이 142건이었는데, 한 바이트도 안 읽는 구독자를 붙이니 1,630건이었습니다. 열 배 넘게 낫습니다. 조금씩 읽는 구독자는 TCP 창을 계속 조금씩 열어 주기 때문에 서버가 계속 "이번엔 보낼 수 있겠지" 하며 매달립니다. 완전히 막힌 구독자는 Tomcat의 20초 제한시간에 걸려 포기당하고, 그 20초 동안 다른 구독자는 처리됩니다. 애매하게 아픈 쪽이 확실히 죽은 쪽보다 나쁩니다.
...
`scripts/run-suite.sh` 에 `RECONNECT_MS` 를 넣어 절단당한 느린 구독자가 다시 붙게 했습니다.
절단이 해소책이 되려면 재접속까지 견뎌야 한다는 것이 이 조건의 질문입니다.
...
정상 구독자의 p95 가 세 조건 모두 1ms 이고 총 수신도 0.2% 안에서 같습니다. 재접속 간격을 100ms 로 좁히면 같은 45초에 8회까지 다시 붙는데(1,000ms 조건은 6회), 그래도 정상 쪽 최대 지연이 16ms 로 재접속 없는 조건과 같습니다. 매번 절단이
다시 작동해 느린 구독자를 끊어 내므로, 재접속이 반복돼도 정상 구독자 쪽 힙과 지연이
흔들리지 않습니다. **절단을 넣어 둔 서버는 재접속 폭풍에 대해서도 방어가 됩니다.**

### 다만 총 전송량은 줄지 않는다

느린 구독자가 받은 바이트가 173KB, 511KB, 1,250KB 로 재접속 횟수에 비례합니다.
...
100ms 조건이 2회, 1,000ms 조건이 6회입니다. 짧은 지연으로 다시 붙으면 서버가 아직 앞
세션의 버퍼를 정리하는 중이라 곧바로 다시 잘리고, 긴 지연 쪽은 매번 깨끗하게 붙어 한
주기를 온전히 살다 잘립니다. **재접속을 빨리 하는 것이 오히려 살아 있는 시간을 줄입니다.**

이 관찰은 조건마다 1회 실행이라 방향까지만 읽어야 합니다.

## 느린 구독자 수를 늘리면 (2026-07-31)

정상 5명 + 느린 1명 한 가지 구성이었습니다. 느린 구독자 비율이 올라가면 결과가 달라질 수 있는데 재지 않았습니다. 클라이언트가 느린 구독자를 하나만 만들 수 있게 짜여 있어 목록으로 고치고, 세 모드를 1·2·4명으로 돌렸습니다. 정상 구독자는 5명 고정입니다.
...
**`terminate` 는 넷까지 견딥니다.** 수신 합이 670,224에서 667,666으로 0.4% 줄어듭니다. 회차 하나짜리 값이므로 이 정도 차이는 편차와 구별되지 않습니다. max 는 10ms 에서 27ms 로 늡니다. **절단이 느린 구독자 수에 대해 무너지지 않습니다.** 넘친 구독자만 끊고 나머지는 그대로 흐르기 때문입니다.

`direct` 대비 43.7배입니다. **한 명일 때 잰 배수가 넷일 때도 유지됩니다.**

**`unbounded` 는 늘수록 나빠집니다.** 1,075,480에서 962,760으로 10% 줄어듭니다. 느린 쪽이 읽은 양이 3,202KB 에서 13,773KB 로 4.3배 늘어나는데, 그만큼 서버가 큐에 쌓아 둔 것입니다. 느린 구독자 1명과 2명 조건은 서버 로그에 `OutOfMemoryError` 가 남았고 4명 조건은 안 남았습니다. **4명 조건이 나아서가 아니라 측정 창 안에 다르게 진행됐기 때문으로 보이며, 이 회차로는 갈라내지 못했습니다.** max 가 11,071ms 에서 755ms 로 줄어든 것도 같은 이유로 인용하지 않습니다.

정상 구독자의 p95 는 아홉 조건 전부 1ms 입니다. **갈리는 것은 p95 가 아니라 max 와 수신 합입니다.** `direct` 의 p95 1ms 만 보면 정상으로 보이는데 실제로는 초당 500건대만 받고 있습니다.

## 못 한 것

- **위 아홉 조건은 각각 1회 실행입니다.** 그리고 정상 구독자는 5명으로 고정했으므로 정상 대 느린 비율을 반대쪽으로 움직여 보지는 않았습니다.
...
- **재접속 폭풍은 느린 구독자 한 명으로만 봤습니다.** 여럿이 동시에 재접속을 반복하는 조건과, 접속 자체가 자원을 소진하는 구간은 만들지 않았습니다. 백오프 전략별 비교도 재지 않았습니다.
