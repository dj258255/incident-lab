...
- **값만 조작된 사고를 안 만들었습니다.** 지급 건수는 맞는데 금액만 부풀려진 경우는
  참조 대사도 개봉 대사도 통계 이탈도 못 잡을 텐데 그 조건은 만들지 않았습니다.
- **쏠린 분포를 제곱근 꼴로 만들었습니다.** 실제 게임 활동이 그 모양인지는 확인하지
  않았습니다. 7절의 오탐 비율은 그 가정 위에 있습니다.
- **파티션 스위치로 보존 정리하는 것을 재지 않았습니다.** 8절은 조사 속도만 봤고,
  파티션의 실제 값어치가 있는 쪽은 안 봤습니다.
- **온라인 빌드의 버전 저장소가 왜 안 잡혔는지 못 밝혔습니다.** 5-2 절에서 22만 8천 행을
  넣는 동안에도 0 이었습니다. 통설과 다른데 "안 잡혔다"까지만 확인했습니다.
- **온라인 빌드의 로그를 두 번째 계기로 못 받쳤습니다.** 트랜잭션 단위 카운터가 온라인
  조건에서만 표본에 안 잡혀, 온라인 두 줄은 파일 카운터 하나로만 서 있습니다.
- **Standard 에디션에서 거부되는 것을 직접 안 봤습니다.** Developer 로만 돌려 `ONLINE = ON`
  이 실제로 어떤 오류로 막히는지는 문서로만 알고 있습니다.
- **시간을 재지 않았습니다.** 조사 쿼리가 실제로 몇 초 걸리는지는 이 랩에서 답할 수
  없습니다. "17분 만에 대응"이라는 실제 사례와 견주려면 그 값이 필요한데 비어 있습니다.
- **탐지를 주기적으로 도는 형태로 안 만들었습니다.** 이 세션의 쿼리는 사고를 안 뒤에
  한 번 도는 것입니다. 매일 돌며 이상을 먼저 알려 주는 배치는 만들지 않았습니다.
- **회수 실행은 [A25](../A25-currency-reclaim-procedure/) 에서 다룹니다.** 이 세션은
  대상을 뽑는 데서 끝납니다.
