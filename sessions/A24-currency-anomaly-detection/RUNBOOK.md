...
| 없고 표가 크다 | **인덱스 생성이 라이브에 주는 영향을 먼저 판단한다.** `ONLINE = ON` 이 되는 에디션인지 확인하고, 안 되면 점검 시간까지 기다릴지 전체 스캔으로 갈지 정한다 |
...
- **참조가 안 남는 사고.** 재화가 어디서 왔는지 로그에 없으면 참조 대사도 원장 대사도
  못 씁니다. 그때는 통계 이탈밖에 없고, 회수 근거로 쓰기 어려우니 정책 판단이 필요합니다.
- **조사 쿼리가 몇 초 걸리는지.** 이 랩은 에뮬레이션이라 시간을 재지 않았습니다.
  실제 환경에서 한 번 재서 이 문서에 적어 두는 편이 좋습니다.
- **인덱스 생성이 라이브에 주는 영향.** 크기는 쟀지만 생성 중 잠김은 재지 않았습니다.
