...
`ONLINE = ON` 의 대가는 에디션 제약과 로그입니다. 2천만 행 표에 657MB 인덱스를 만들며
실측한 값입니다(동시 쓰기 없음).

| 복구 모델 | ONLINE | 로그 | 인덱스 크기 대비 |
|---|---|---|---|
| SIMPLE | OFF | 11MB | 2% |
| SIMPLE | ON | 99MB | 15% |
| FULL | OFF | 712MB | 108% |
...
**복구 모델을 먼저 봅니다.** SIMPLE 과 FULL 은 자릿수가 다릅니다. 최소 로깅은 SIMPLE 에서
**온라인 빌드에도 걸립니다.** FULL 이라면 빌드 중 로그 백업 주기를 당겨 둡니다. 안 그러면
백업 사이에 이 값이 통째로 쌓입니다.

**tempdb 는 `SORT_IN_TEMPDB` 를 켤 때만 봅니다.** 기본값 OFF 에서는 정렬이 인덱스가 들어갈
파일 그룹에서 일어나 tempdb 가 안 움직였습니다(3MB). 켜면 인덱스 크기만큼 잡습니다(699MB).
기본값으로 만든다면 봐야 할 여유는 tempdb 가 아니라 **대상 파일 그룹**입니다.

> 로그 사용량은 `sys.dm_io_virtual_file_stats` 의 `num_of_bytes_written` 차분으로 잽니다.
> `sys.dm_db_log_space_usage` 는 "지금 얼마나 차 있나"라 체크포인트가 지나가면 회수됩니다.
> 절대값은 표 크기에 딸린 값이라 옮겨 쓸 수 없고, **재는 방법과 인덱스 크기 대비 배수**를
> 옮겨 씁니다.
...
활동이 소수에 쏠린 분포(정상 20,000 계정, 이상 30 계정)에서 기준을 올려 가며 쟀습니다.

| 기준 | 지목 | 적발 | 오탐 | **지목 중 무고** |
|---|---|---|---|---|
| 2시그마 | 173 | 30/30 | 143 | 82.7% |
| 3시그마 | 129 | 30/30 | 99 | 76.7% |
| 4시그마 | 86 | 30/30 | 56 | 65.1% |
| 5시그마 | 65 | 30/30 | 35 | 53.8% |

**어느 기준에서도 오탐이 0이 되지 않습니다.** 활동이 쏠린 분포에서 상위 정상 이용자와
어뷰저는 같은 꼬리에 있습니다. 5시그마까지 올려도 지목된 계정의 절반이 넘게 무고합니다.
...
사고가 나기 전에 정해 두면 조사가 훨씬 싸집니다.

| 무엇 | 왜 |
|---|---|
| **원장의 클러스터드 키를 시간 순으로 잡는다** | 조사 창을 좁히는 힘의 거의 전부가 여기서 온다. 실측에서 114,353 → 638 |
| 조사용 인덱스를 하나 둔다 | 638 에서 482 로 더 내려간다. 사고 뒤에 만들면 쓰기가 막힌다 |
| 파티션은 보존 정리를 위해 둔다 | **조사 속도에는 도움이 안 됐다**(638 대 657). DELETE 대신 파티션 스위치로 정리하려면 필요하다 |

조사 쿼리는 파티션 키·클러스터드 키를 **함수로 감싸지 않습니다.** `CONVERT(date, created_at) = ...`
는 읽기에 자연스럽고 결과도 맞지만 범위를 못 자릅니다. 실측에서 657 이 7,499 가 됐습니다.
...
- **보상 테이블 안에 머무는 조작의 회수 근거.** 지급액이 전부 합법이면 초과분이라는
  개념이 없어 DB 로는 회수액을 못 냅니다. 근거는 지급 로직의 코드나 서버 로그에서
  와야 하고 그것은 DB 밖입니다.
- **실제 게임 활동의 분포.** 시그마 기준은 분포 모양이 정하는데 그 모양을 이 랩은
  모릅니다. 운영에서는 자기 데이터의 분포를 먼저 그려 보고 기준을 정합니다.

---

## 대사 항목은 축이 둘이다

건수를 세는 대사만 갖추면 **금액만 조작된 사고를 통째로 놓칩니다.** 실측에서 참조
중복·개봉 대조·참조 결측 셋이 모두 0/50 이었습니다. 셋이 다 건수를 보기 때문입니다.

| # | 대사 | 축 |
|---|---|---|
| 1 | 같은 참조로 두 번 지급됐는가 | 건수 |
| 2 | 개봉 수와 지급 수가 맞는가 | 건수 |
| 3 | 참조가 없는 지급이 있는가 | 건수 |
| 4 | **지급액이 보상 테이블 안에 있는가** | **금액** |

```sql
-- 4번. 줄 수 있는 값의 목록은 손으로 적지 말고 사고 이전 데이터에서 읽는다.
SELECT DISTINCT account_id
  FROM currency_ledger
 WHERE reason = 1 AND created_at >= @from AND created_at < @to
   AND delta NOT IN (SELECT DISTINCT delta FROM currency_ledger
                      WHERE reason = 1 AND created_at < @incident_start);
```

**조작이 규칙 밖으로 나가면 결정론적으로 잡히고 회수액도 정확히 나옵니다. 규칙 안에
머물면 어느 결정론적 방법도 못 잡고 통계로 가야 하며, 거기서는 회수 근거를 못 만듭니다.**

---

## 보존 정리는 SWITCH 로 한다

466만 행을 없애는 데 든 비용입니다.

| 방법 | 로그 | 최대 락 | 락 모양 |
|---|---|---|---|
| `DELETE` 4,000행씩 | 1,031MB | 6,261 | `IX` `X` (승격) |
| `SWITCH` | 0MB | 5 | `Sch-M` |

```sql
-- 받을 표는 평소에 만들어 둔다. 사고 중에 급히 맞출 수 있는 조건이 아니다.
CREATE TABLE ledger_archive (...같은 컬럼...,
    CONSTRAINT CK_archive_range CHECK (created_at >= @low AND created_at < @high)
) ON [PRIMARY];
CREATE CLUSTERED INDEX CX_archive ON ledger_archive (created_at, ledger_id) ON [PRIMARY];

ALTER TABLE currency_ledger_p SWITCH PARTITION @n TO ledger_archive;
```

| 방향 | 조건 |
|---|---|
| 나갈 때 | 구조가 같고 같은 파일 그룹, 받을 표가 비어 있으면 된다. **CHECK 는 불필요** |
| 들어올 때 | 모든 행이 그 파티션 범위 안임을 **CHECK 로 증명**해야 한다. 한쪽 경계만 걸면 `Msg 4972` |
...
```sql
-- 워터마크 이동과 경보 기록을 같은 트랜잭션에 둔다.
BEGIN TRAN;
    INSERT INTO detect_alert (...)  -- 이미 있는 경보는 NOT EXISTS 로 거른다
    ...;
    UPDATE detect_state
       SET watermark = DATEADD(minute, -@lag, @to)   -- 창 끝이 아니라 뒤로 물린다
...
| 항목 | 이유 |
|---|---|
| 워터마크를 표에 두고 같은 트랜잭션에서 옮긴다 | 밖에서 옮기면 죽었을 때 그 창을 건너뛴다 |
| **창 끝이 아니라 뒤로 물린다** | `created_at` 은 일어난 시각이지 도착한 시각이 아니다 |
| 지연 폭은 **도착 지연보다 넓게** | 실측에서 폭이 좁아 지연을 둬도 놓쳤다 |
| 유일 제약 (탐지기, 계정, 참조) | 겹쳐 읽어도 중복 경보가 안 쌓인다 |

**경보는 조사 대상이지 회수 근거가 아닙니다.** 배치가 하는 일은 사람을 부르는 데까지입니다.

---

## 낮은 에디션

`ONLINE = ON` 은 `Msg 1712` 로 거부됩니다("Online index operations can only be
performed in Enterprise edition"). **파티션·필터드 인덱스·데이터 압축·확장 이벤트는
Express 에서도 됩니다**(2016 SP1 이후). 에디션 때문에 미리 접을 자리는 좁고,
실제로 걸리는 것은 온라인 빌드입니다. 그렇다면 답은 인덱스가 아니라
**클러스터드 키를 평소에 시간 순으로 잡는 것**입니다.
