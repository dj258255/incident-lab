...
커밋 간격이 아니라 커밋 문장 자체의 왕복을 100건씩 쟀습니다.

| 조건 | 중앙값 | p95 | 최대 |
|---|---|---|---|
| 비동기 | 5.17ms | 6.39ms | 8.70ms |
| 반동기 `AFTER_SYNC` | 5.29ms | 6.29ms | 11.72ms |
| 반동기 `AFTER_COMMIT` | 5.16ms | 6.15ms | 8.14ms |

**세 조건이 구별되지 않습니다.** 커밋 간격으로 재던 것을 커밋 자체로 바꿔 봐도 같습니다.
두 노드가 같은 호스트의 컨테이너라 복제 왕복이 커밋 자체의 비용에 묻힙니다. **반동기의
...
여기서 8이라는 숫자는 동시 쓰기 수입니다. 쓰기 하나가 커밋 뒤에 매달리므로, 매달린 시점에 커밋을 마친 쓰기의 수만큼 노출됩니다. **동시 쓰기가 800개인 서버라면 이 창에 800건이 보였다가 사라집니다.** 창의 길이는 유실 건수를 안 정합니다. 정하는 것은 그 창에 커밋을 마친 쓰기의 수입니다.

`AFTER_COMMIT` 이 `AFTER_SYNC` 보다 빠른 것은 맞습니다. 커밋을 먼저 끝내고 ack 를 기다리므로 다른 세션의 잠금 대기가 짧아집니다. **그 속도의 값이 위 표의 마지막 열입니다.**

### 이 조건을 세우는 데 세 번 실패했습니다
