...
분단과 강제 종료를 반동기에서 반복했습니다. **창 길이는 조건마다 다릅니다.** A는 분단부터 강제 종료까지 15초였고 C는 18초인데 그중 앞 8초는 반동기가 ack를 기다리며 커밋을 막고 있었습니다. 이 글의 식(유실 = 창 × 그 창에 커밋을 마친 쓰기 수)에서 창이 변수이므로, 아래 표의 555건과 334건을 창이 같은 조건의 비교로 읽으면 안 됩니다.
...
커밋 간격이 아니라 커밋 문장 자체의 왕복을 100건씩 쟀습니다. **잰 것은 정확히는 접속을 포함한 INSERT 1건입니다.** 측정 루프가 반복마다 `mysql` 클라이언트를 새로 띄우므로 fork/exec와 TCP 접속과 인증이 매번 들어갑니다. 아래 표의 5ms대 바닥은 대부분 그 비용입니다.
...
### 2. AFTER_COMMIT은 읽힌 값을 잃는다

복제망을 끊어 ack가 오지 않는 상태에서 커밋을 던지고, 그 커밋이 매달린 동안 **다른
...
복제망을 끊어 ack가 오지 않게 한 뒤, 관찰자들이 그동안 실제로 읽은 행을 전부 적어 두고, 소스를 죽이고 복제본을 승격해 그중 몇 건이 사라졌는지 셌습니다.
