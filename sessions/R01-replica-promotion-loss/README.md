...
초기 회차의 호스트 사양을 남기지 않은 것은 그 회차를 이 저장소의 다른 세션과 절대 시간으로 비교할 수 없다는 뜻입니다. 저장소에는 서로 다른 호스트가 섞여 있고 그때가 어느 쪽이었는지는 확인되지 않습니다. 추정해서 채우지 않았습니다.
...
분단은 10초 지점인데 레플리카의 IO 스레드는 29초 지점에야 연결이 죽었음을 알아챕니다(`replica_net_timeout` 경과). **그 19초 동안 지표는 0입니다.** 커밋 555건이 유실 대기열에 쌓이는 동안 복제 지연 대시보드는 건강해 보입니다. 이 지표로 페일오버 안전 여부를 판단하면 안 되는 이유이고, 실제 격차는 GTID 집합 비교로 봐야 합니다.
...
이 절에서는 커밋 지연을 따로 계측하지 않았습니다(6절에서 커밋 자체를 100건씩 다시 잽니다). 남은 것은 커밋 간격뿐이고, 그 값은 비동기와 반동기가 같았습니다. 양쪽 다 중앙값 27밀리초, p95 30밀리초입니다.

같다는 결과를 "왕복 비용이 없다"로 읽으면 안 됩니다. 클라이언트가 커밋 사이에 20밀리초를 쉬고 타임스탬프 해상도가 1밀리초라, 왕복이 그보다 작으면 이 장치로는 보이지 않습니다. 게다가 두 노드가 같은 호스트의 컨테이너라 이 왕복은 실제 가용영역 사이의 왕복보다 짧습니다. 정리하면 **반동기의 평상시 지연 비용은 이 환경에서 잴 수 없습니다.** 6절에서 계측 장치를 커밋 자체로 바꿔 다시 쟀는데 거기서도 세 조건이 안 갈렸습니다.
...
분단과 강제 종료를 반동기에서 반복했습니다. **창 길이는 조건마다 다릅니다.** A 는 분단부터 강제 종료까지 15초였고 C 는 18초인데 그중 앞 8초는 반동기가 ack 를 기다리며 커밋을 막고 있었습니다. 이 글의 식(유실 = 창 × 그 창에 커밋을 마친 쓰기 수)에서 창이 변수이므로, 아래 표의 555건과 334건을 창이 같은 조건의 비교로 읽으면 안 됩니다.
...
이 세션은 병합만 해 봤습니다. 6절에서 옛 소스의 차집합을 뽑아 유실 50행을 전부 회수했습니다. 승격본이 이미 같은 키에 새 값을 쓴 경우의 충돌 처리는 그 시나리오를 만들지 않았고, 백업은 주기도 복원 시간도 재지 않았습니다. 그래서 이 글이 수치로 말할 수 있는 것은 "얼마나 잃는가"와 "가장 좋은 조건에서 얼마나 되찾는가"까지입니다.
...
## 6. AFTER_COMMIT 대조, 평상시 지연, 복구 경로

`scripts/exp-after-commit.sh`, 원문은 `results/exp-after-commit.txt`.

### 1. 반동기의 평상시 커밋 지연

커밋 간격이 아니라 커밋 문장 자체의 왕복을 100건씩 쟀습니다. **잰 것은 정확히는 접속을 포함한 INSERT 1건입니다.** 측정 루프가 반복마다 `mysql` 클라이언트를 새로 띄우므로 fork/exec 와 TCP 접속과 인증이 매번 들어갑니다. 아래 표의 5ms대 바닥은 대부분 그 비용입니다.

| 조건 | 중앙값 | p95 | 최대 |
|---|---|---|---|
| 비동기 | 5.22ms | 6.22ms | 7.67ms |
| 반동기 `AFTER_SYNC` | 5.40ms | 6.34ms | 7.69ms |
| 반동기 `AFTER_COMMIT` | 5.24ms | 6.45ms | 6.89ms |

**세 조건이 구별되지 않습니다.** 커밋 간격으로 재던 것을 커밋 자체로 바꿔 봐도 같습니다.
이유가 둘입니다. 두 노드가 같은 호스트의 컨테이너라 복제 왕복 자체가 짧고, 계측 장치가
반복마다 접속을 새로 맺어 그 비용이 왕복보다 큽니다. **반동기의
평상시 비용은 이 환경에서 잴 수 없다는 것이 결론입니다.** 실제 가용영역 사이에서는 왕복이
밀리초 단위로 붙으므로 다른 그림이 나옵니다.

### 2. AFTER_COMMIT 은 읽힌 값을 잃는다

복제망을 끊어 ack 가 오지 않는 상태에서 커밋을 던지고, 그 커밋이 매달린 동안 **다른
세션이 그 값을 읽을 수 있는지** 봅니다. 그다음 소스를 죽이고 복제본을 승격합니다.

| `wait_point` | 매달린 커밋을 다른 세션이 읽는가 | 승격 후 남는가 |
|---|---|---|
| `AFTER_SYNC` | **0건** | 0건 |
| `AFTER_COMMIT` | **1건** | **0건** |
...
`AFTER_COMMIT` 은 먼저 엔진에 커밋하고 그 뒤에 ack 를 기다립니다. 그 창에서 다른 세션이
값을 읽었고, 소스가 죽자 **읽힌 값이 사라졌습니다.** 사용자 화면에 이미 뜬 후원이
없어지는 것이 이 한 줄의 뜻입니다.

이것이 "lossy failover 가 가능한 구성"의 실체입니다. 유실량이 더 큰 것이 아니라
**유실의 성격이 다릅니다.** 비동기의 유실은 "성공 응답을 받았는데 사라진 것"이고,
`AFTER_COMMIT` 의 유실은 거기에 "제3자가 이미 본 것"이 더해집니다.

### 3. 복구: 차집합을 뽑아 병합한다

비동기로 두고 복제망을 끊은 채 50건을 넣은 뒤 소스를 죽여 유실을 만들었습니다.

| 단계 | 값 |
|---|---|
| 분단 직전 양쪽 행 수 | 600 / 600 |
| 분단 중 소스에만 들어간 쓰기 | 50건 (클라이언트는 전부 성공을 받음) |
| 승격 후 옛 소스 / 새 소스 | 650 / 600 |
| **유실량** | **50행** |
| 병합 후 새 소스 | 650 |
| **회수량** | **50행** |
...
**유실량을 재는 것과 복구량을 재는 것은 다른 질문이고, 복구량이 100%라는 것이 안전을
뜻하지 않습니다.** 이 실험은 조건이 가장 좋을 때의 상한을 보인 것입니다.
...
## AFTER_COMMIT 을 부하 아래에서 (2026-07-31)

`AFTER_COMMIT` 대조는 관찰자 한 명, 커밋 한 건이었습니다. 한 건짜리 관측은 "이런 일이 가능하다" 까지만 말합니다. 쓰기 8개와 관찰자 4명을 두고 12초 창에서 다시 쟀습니다.

복제망을 끊어 ack 가 오지 않게 한 뒤, 관찰자들이 그동안 실제로 읽은 행을 전부 적어 두고, 소스를 죽이고 복제본을 승격해 그중 몇 건이 사라졌는지 셌습니다.

| wait_point | 소스에 들어간 행 | 관찰자가 읽은 행 | 승격 후 남은 행 | **읽혔는데 사라진 행** |
|---|---|---|---|---|
| `AFTER_SYNC` | 0 | 0 | 0 | **0** |
| `AFTER_COMMIT` | 8 | 8 | 0 | **8** |

**`AFTER_SYNC` 는 소스에 들어간 행이 0 입니다.** 쓰기 8개가 전부 커밋에서 매달렸고, ack 를 받기 전에는 엔진에 커밋하지 않으므로 관찰자에게 아무것도 안 보입니다. 승격 후에도 0 이고 유실은 0 입니다. **못 본 것은 사라져도 아무도 모릅니다.**

**`AFTER_COMMIT` 은 8건이 보였다가 사라집니다.** 쓰기 8개가 각각 커밋을 마치고 ack 를 기다리는 동안, 관찰자 4명이 그 8건을 읽었습니다. 승격 후 복제본에는 0 건입니다. **읽힌 값 전부가 사라졌습니다.**
...
`AFTER_COMMIT` 이 `AFTER_SYNC` 보다 빠른 것은 맞습니다. 커밋을 먼저 끝내고 ack 를 기다리므로 다른 세션의 잠금 대기가 짧아집니다. **그 속도의 값이 위 표의 마지막 열입니다.**
...
두 번째는 반동기 상태를 켠 직후에 읽은 것이었습니다. `Rpl_semi_sync_source_status` 는 반동기 복제본이 붙어 ack 를 한 번 보내야 `ON` 이 됩니다. 설정 직후에는 `OFF` 입니다. 탐침 쓰기를 하고 `ON` 이 될 때까지 기다리게 고쳤습니다.

세 번째는 조건 사이의 상태 누수였습니다. 첫 조건이 복제본을 승격하며 `RESET REPLICA ALL` 을 걸어 다음 조건의 복제가 안 붙었고, 복제본이 첫 조건의 표를 그대로 들고 있었습니다. 소스에 8행뿐인데 남은 행이 400 으로 찍혔습니다.
...
지금은 네 자리에 전부 확인이 들어 있고, 확인이 안 서면 실행을 멈춥니다. **아무 일도 안 일어나면 유실은 항상 0 입니다.** 0 을 결과로 쓰려면 조건이 섰다는 증거가 함께 있어야 합니다.

## 못 한 것

- **위 표는 조건마다 1회 실행이고 동시 쓰기 8개 하나입니다.** 동시 쓰기 수를 바꿔 가며 유실 건수의 곡선을 그리지는 않았습니다.
- **호스트 사양을 기록하지 않았습니다.** `uname -srm`, `nproc`, `free -g`를 남기지 않아 이 세션이 어느 장비에서 돌았는지 확인되지 않습니다. 다른 세션과 절대 시간을 비교할 수 없습니다.
- **반동기의 평상시 지연 비용은 이 환경에서 잴 수 없습니다.** 커밋 자체를 100건씩 재도 세 조건이 구별되지 않습니다(중앙값 5.17/5.29/5.16ms). 두 노드가 같은 호스트라 복제 왕복이 커밋 비용에 묻힙니다. 가용영역을 나눈 환경이 필요합니다.
- **RDS에서 검증하지 않았습니다.** 5절의 대응 관계는 AWS 문서와 대조한 것이고, 실제 리드 리플리카 승격이나 Multi-AZ DB 클러스터 페일오버를 돌려 본 것이 아닙니다.
- **백업 복원 경로는 실행하지 않았습니다.** 차집합 병합은 재봤지만 옛 소스가 아예 못 뜨는 경우의 복구는 다루지 않았습니다. 그쪽은 A23 세션의 주제입니다.
...
- **AFTER_COMMIT 대조는 관찰자 한 명, 커밋 한 건입니다.** 읽힌 값이 사라진다는 것은 보였지만 부하 아래에서 몇 건이 그렇게 되는지는 재지 않았습니다.
