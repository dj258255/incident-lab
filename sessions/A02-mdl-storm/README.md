...
자체가 흔들리므로, 세 회차를 더 돌려 확인했습니다. 12코어 맥에서 `tools/repeat-runs.sh`로
돌린 값입니다.
...
"run0의 32.3%가 편차였다"를 장비 간 비교로 증명한 것은 아닙니다. 말할 수 있는 것은
...
**`INPLACE` 는 시작하자마자 배타 락을 요구합니다.** 25.1초에 `EXCLUSIVE:PENDING` 이 뜨고 그때부터 조회가 막힙니다. 인덱스 빌드는 락을 얻은 **뒤에** 합니다. 그래서 완료가 가장 늦습니다(22.58초).
