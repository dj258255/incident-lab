...
| 같은 구조의 실사건 | E1 | Railway, GoCardless. **다만 PostgreSQL이고 락 이름도 다릅니다** |
...
| `/other` | 645.9ms | **2,009.4ms** | 획득 타임아웃 2초. **DB에 닿지도 못함** |
...
2. **동시 실행 상한을 누적 스레드 수로 셌습니다.** `threads` 리스트가 줄어들지 않아 상한
   600이 총 발사 수를 제한했고, 대조군에서도 200건이 발사되지 못했습니다. 조건 사이 비교가
   깨집니다. `BoundedSemaphore` 로 바꿨습니다.

3. **`load` 이미지에 `cryptography` 가 없었습니다.** MySQL 8의 `caching_sha2_password` 는
   서버 캐시가 비어 있을 때 RSA 공개키 교환을 하고 pymysql이 그것을 하려면 이 패키지가
   필요합니다. 한 번 성공하면 캐시 덕에 없어도 붙으므로 **볼륨을 새로 만든 회차에서만
   조용히 깨집니다.** 반복 측정에서 특히 위험한 결함이라 Dockerfile에 넣었습니다.
...
타임라인은 앞 절들과 같습니다. t=15에 롱 트랜잭션이 `SELECT` 후 커밋하지 않고, t=25에 DDL이 들어오고, t=45에 롱 트랜잭션이 커밋합니다.
...
- **6절의 풀은 최소 구현입니다.** `queue.Queue` 기반이라 HikariCP의 `maxLifetime`, `leakDetectionThreshold`, 하우스키핑이 없습니다. 획득 타임아웃도 기본값 30초가 아니라 2초로 줄여 잡았습니다.
