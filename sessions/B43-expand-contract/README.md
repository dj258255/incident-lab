...
**범위의 위쪽 끝은 원문이 없습니다.** `results/` 에 통째로 남긴 것은 마지막 회차 로그(`full-run.txt`)뿐이라, 표의 왼쪽 값(11,316.601 ms 등)은 그 파일에서 확인되지만 오른쪽 끝(27,989.225 ms 등)은 앞 네 회차의 콘솔 출력이고 그것을 저장하지 않았습니다. **인용할 수 있는 값은 왼쪽이고, 범위는 "이만큼 흔들렸다"는 기록으로만 읽어야 합니다.** 회차 로그를 통째로 남기는 것으로 절차를 고쳤습니다.
