...
같은 300만 행에서 잰 값입니다. 비교하는 축은 전체 소요 시간이 아니라 최대 락 보유 시간과 그동안 SELECT가 막힌 시간입니다. 5회차 실행의 값이고, 회차마다 흔들린 항목은 칸 안에 다섯 회차의 관측 범위를 같이 적었습니다.
...
|---|---|---|---|---|---|
| 1 | 125.3ms | 150.2ms | 1.2배 | 114,676 | **170,528** |
| 2 | 122.9ms | 156.4ms | 1.3배 | 114,672 | **192,220** |
| 3 | 124.3ms | 147.0ms | 1.2배 | 114,668 | **194,692** |

**기준선의 read 블록이 세 회차에서 114,668~114,676 으로 사실상 같습니다.** 같은 조회가
같은 테이블을 세는 것이니 당연합니다. 그런데 백필이 도는 동안 같은 조회의 read 블록이
**170,528 에서 194,692 로 1.5배에서 1.7배 늘어납니다.**
