...
4절은 백필 중 조회가 느려지는 것을 관측하고 "락은 아니다"까지 배제했습니다.
그러면 "그럼 무엇이냐"가 남습니다. `EXPLAIN (ANALYZE, BUFFERS)` 로 같은 조회의
버퍼 접근을 재고, `pg_stat_io` 로 구간 I/O를 함께 봤습니다.
...
**기준선의 read 블록이 세 회차에서 114,664~114,672로 사실상 같습니다.** 같은 조회가
같은 테이블을 세는 것이니 당연합니다. 그런데 백필이 도는 동안 같은 조회의 read 블록이 **185,040에서 194,424로 1.6배에서 1.7배 늘어납니다.**
...
4절이 관측한 것과 같은 방향이고, 이제 원인이 붙었습니다.
...
으로 다음 쓰기가 그 자리를 재사용할 수 있게는 되지만, 디스크에서 711MB가 줄어들지는
...
쥔 채 도는 동안의 `SELECT` 가 10,936ms이고 기준선이 245~311ms 인 비율입니다.
...
| autovacuum이 걸리는 죽은 튜플 문턱 | 602,909 |
...
- **백필 직후 0~60초에 DDL을 걸면** autovacuum이 아직 안 붙었으므로 락 대기가 없습니다. **이 창이 가장 안전합니다.**
- **60~124초 사이에 걸면** 6절에 적은 상황이 됩니다. autovacuum이 표를 쥐고 있고, 서버가 그것을 취소해 주기는 하지만 그 판단이 `deadlock_timeout`(기본 1초) 뒤에 일어나므로 1초가 그냥 나갑니다.
- **124초 뒤에 걸면** 다시 깨끗합니다.
...
**절대값은 앞 표와 나란히 놓으면 안 됩니다.** 호스트가 다릅니다. 보유 시간이 11,316ms에서 3,648ms로, 기준선이 245~311ms에서 50ms로 내려갔습니다.
...
- **표 하나, 1회 실행입니다.** 300만 행에서 vacuum이 64.3초였는데 표 크기를 바꿔 가며 그 길이가 어떻게 가는지는 안 봤습니다.
