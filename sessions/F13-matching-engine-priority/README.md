...
**병렬 위반은 호가 단위와 단조 관계가 없습니다.** 1,524에서 102까지 오르내립니다. 스레드가 큐에서 꺼낸 뒤 기록까지 걸리는 시간이 정하므로 이 값은 스케줄링 잡음이고, 방향을 읽으면 안 됩니다.
...
**이 세션이 줄곧 써 온 "인접 쌍 위반 수" 라는 지표가 이런 통짜 오류를 못 잡습니다.** 앞 절들의 위반 건수는 순서가 국소적으로 흐트러진 경우를 재는 데는 맞지만, 규칙 한 층이 통째로 빠진 경우에는 거의 0 을 돌려줍니다.
