...
| 지금 알면 되는 것 | 근거 |
|---|---|
| **원자 구간이 없으면 체결의 27~75%가 시간우선을 위반한다.** 접수 순서를 시퀀스로 찍는 일과 그 순서대로 체결하는 일은 별개다 | 조건 8개 × 각 51회 ([근거](#5-재계측)) |
| **원자 구간을 넣은 세 조건은 153회 전부 0건** | ([근거](#5-재계측)) |
| **해소가 기준보다 빨랐다.** 원자 구간의 순수 비용은 1.06~1.78배인데 함께 바꾼 큐 교체가 1.4~1.8배 이득이라 상쇄된다 | ([근거](#6-예상과-달랐던-점)) |
| **규칙을 넓히면 같은 실행에서 위반이 늘어난다.** 공급이 모자란 구간에서 누가 받는지가 구현 부산물로 정해진다 | ([근거](#7-원칙을-넷으로-늘리면)) |
| **"인접 쌍 위반 수"라는 지표가 통짜 오류를 못 잡는다.** 지표 자체의 한계를 실측으로 확인했다 | ([근거](#호가-단위와-시장가를-넣으면)) |
...
`app/Principles.java`, 결과는 `results/principles.txt` 입니다. Matching.java는 그대로
두었고 옆에 붙는 파일입니다.
...
거래소 안내의 서술을 코드로 옮긴 것이라 호가 단위와 시장가가 들어 있지 않았습니다. 둘을 넣으면 이 세션의 결론이 바뀌는지 봤습니다. 주문 20,000건, 기준가 50,000원, 상하한 폭 ±15,000원입니다.

### 호가 단위가 성길수록 3·4번 원칙이 개입할 자리가 늘어납니다
...
**호가 단위가 1,000배가 되면 동률이 0.0%에서 9.8%가 됩니다.** 가격이 격자 위에만 있으므로 격자가 성길수록 같은 칸에 주문이 몰립니다. 가격우선으로 안 갈리는 쌍이 그만큼 늘고, 거기서 위탁매매우선과 수량우선이 판정합니다.

**병렬 위반은 호가 단위와 단조 관계가 없습니다.** 71에서 1,121까지 오르내립니다. 스레드가 큐에서 꺼낸 뒤 기록까지 걸리는 시간이 정하므로 이 값은 스케줄링 잡음이고, 방향을 읽으면 안 됩니다.

실제 한국거래소의 호가 단위는 가격대별로 1원부터 1,000원까지입니다. **같은 엔진이라도 저가주와 고가주에서 3·4번 원칙이 걸리는 빈도가 다릅니다.**
...
시장가는 가격이 없으므로 가격우선의 비교 대상이 아닙니다. 시장가끼리는 전부 같은 층에 있고, 그 안에서는 시간·위탁·수량이 판정합니다. **비율이 오를수록 동률이 27.1%까지 오르는 것이 그 뜻입니다.**

호가 단위 50원 조건에서 시장가 없이 0.5% 이던 동률이 시장가 절반에서 27.1%가 됩니다. **시장가는 호가 단위보다 큰 축입니다.**

### 시장가를 층으로 안 두면 지표가 그것을 못 잡습니다

시장가를 지정가와 같은 축에서 다루면(가격을 0원으로 두면) 규정과 어긋납니다. 그 순서를 규정 비교자로 채점했습니다.
...
**1건입니다.** 그런데 같은 순서를 다르게 세면 이렇습니다.

- 시장가 주문 3,994건 중 지정가에 밀린 것 **3,994건(100.0%)**
- 시장가 한 건이 밀린 지정가 수: 평균 **16,006건**, 최대 16,006건

시장가가 통째로 뒤로 몰려 경계가 한 곳뿐이라 인접 쌍으로는 1건입니다. 실제로는 **시장가 전부가 지정가 전부에 밀렸습니다.**

**이 세션이 줄곧 써 온 "인접 쌍 위반 수" 라는 지표가 이런 통짜 오류를 못 잡습니다.** 앞 절들의 위반 건수는 순서가 국소적으로 흐트러진 경우를 재는 데는 맞지만, 규칙 한 층이 통째로 빠진 경우에는 거의 0을 돌려줍니다.

그리고 이것은 **단일 스레드에서 났습니다.** 동시성 문제가 아니라 규칙을 잘못 옮긴 것인데, 병렬 매칭의 위반과 겉모습이 같습니다. **위반 건수만 보고 스레드를 줄이는 쪽으로 가면 원인을 못 찾습니다.**

## 한계

- **호가 단위와 시장가는 각 조건 1회 실행이고 시드가 고정입니다.** 병렬 위반은 스케줄링 잡음이라 회차를 늘려도 방향이 안 나올 값입니다. 단일가매매의 배분 규칙은 여전히 안 넣었습니다.
