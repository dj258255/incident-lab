...
첫 줄의 "전환 계획 수립"을 한 줄로 넘기면 안 되는 이유는 RDS 절에 따로 적었습니다. utf8mb4 전환은 스토리지 결정입니다.
...
- **utf8mb3 치환(지뢰 1).** 해소는 `ALTER TABLE ... CONVERT TO CHARACTER SET utf8mb4`이고 **6절에서 100만 행 테이블에 실제로 돌렸습니다**(COPY 4.6초). 여기서 여전히 확인하지 않은 것은 전환 뒤 같은 INSERT가 통과하는지입니다. 다만 이미 저장된 행은 전환해도 복구되지 않습니다. 출력이 `좋은 방송 ? 후원했어요`라 원본 바이트가 남아 있지 않기 때문입니다. 치환은 잃어버린 뒤에 고칠 수 없습니다.
 ...
 **이 세션은 RDS에서 아무것도 돌리지 않았습니다.** MySQL 8.4.3 도커 컨테이너 하나가 전부이고, 아래는 AWS 문서와 대조해 맞춘 것이지 실측이 아닙니다.
...
**`INPLACE` 를 에러 1846으로 거부합니다.** A01의 `ALTER TYPE` 과 같은 자리이고, 같은
...
크기가 변하지 않은 것은 실제 데이터가 ASCII와 한글이라 저장 바이트가 같기 때문입니다.
...
WordPress가 `VARCHAR(191)` 을 쓰던 이유가 이것입니다**(767 ÷ 4 = 191).

### 2. DST 전환의 결손 시각과 중복 시각

미국 동부 기준입니다. 타임존 테이블은 `mysql_tzinfo_to_sql` 로 1,792개를 적재했습니다.

**결손 시각.** 2026-03-08 02:00에 03:00으로 건너뜁니다. 02:30은 존재하지 않습니다.
...
**에러도 NULL도 아닙니다.** 02:30이 07:00을 돌려주는데 이는 03:00과 같은 순간입니다.
존재하지 않는 시각이 조용히 밀립니다.

**중복 시각.** 2026-11-01 02:00에 01:00으로 되돌립니다. 01:30이 두 번 옵니다.
...
01:30은 EDT 쪽(`05:30`)만 나옵니다. EST 쪽 `06:30` 은 이 문법으로 도달할 방법이 없습니다.

**저장한 뒤에는 되돌릴 수 없습니다.** `DATETIME` 과 `TIMESTAMP` 양쪽에 같은 01:30을 넣고
...
이 서버의 tzdata는 `tzdata-2024b` 라 변경이 반영돼 있습니다. **tzdata가 2022b 이전인
서버에서는 두 값이 둘 다 5로 나옵니다.** 에러도 NULL도 아니고 한 시간 어긋난 값이
조용히 나옵니다.

이것이 RDS 절이 말한 "NULL보다 조용한 실패"의 실체입니다. `mysql.time_zone_name` 이
비어 있으면 `CONVERT_TZ` 가 NULL을 돌려주므로 그건 눈에 띕니다. **적재는 돼 있는데
낡은 경우가 더 위험합니다.**
...
| 전환 중 | **9ms와 4,124ms** (표본 2건) |

**실패가 0건입니다.** 쓰기가 거부된 것이 아니라 기다렸습니다. 그리고 기다린 시간이
전환 창 안의 표본은 2건이고 9ms와 4,124ms입니다. 긴 쪽 4,124ms가 `ALTER` 소요 4,193ms와 거의 같습니다. **전환이 시작될 때 들어온 쓰기는
전환이 끝날 때까지 통째로 멈춥니다.**
...
원본에 공유 락을 잡아 문서상 `ALGORITHM=COPY` 는 읽기를 통과시키고 쓰기만 멈추므로 조회 기반 헬스체크는 통과할 것으로 보입니다. 다만 이 랩의 부하 루프는 INSERT만 던지고 SELECT 프로브가 없어 읽기 쪽은 재지 않았습니다. 락 대기 타임아웃(`innodb_lock_wait_timeout`, 기본 50초)보다 `ALTER` 가
길어지면 그때부터 에러 1205로 바뀝니다. 이 랩은 4.2초라 그 전입니다.
...
서버 두 대의 tzdata가 다르면 이 두 열이 **서로 다른 서버의 답**이 됩니다. 로그의 시각과
DB의 시각이 한 시간 어긋나는 사고가 여기서 나옵니다. 한 서버 안에서 재현했으므로
...
MySQL은 결손 시각에 에러도 NULL도 아닌 값을 돌려줬습니다. 자바는 어떤지 봤습니다.
...
**예외 없이 03:30으로 옮겨집니다.** `LocalDateTime.of(2026,3,8,2,30).atZone(ny)` 가
조용히 한 시간 뒤를 돌려줍니다. MySQL이 `03:00` 과 같은 순간을 돌려준 것과 같은
...
**오프셋을 버리면 두 순간이 한 값으로 뭉갭니다.** MySQL의 `DATETIME` 이 정확히 이
...
**`TIMESTAMP` 는 세션 시간대로 해석해 UTC로 바꿔 담습니다.** 뉴욕 12:00이 UTC 16:00이 됩니다. EDT가 UTC-4 이므로 맞습니다.

**중복 시각에서 드라이버가 앞의 것을 고릅니다.** 01:30이 05:30으로 저장됐습니다. EDT(UTC-4) 기준입니다. EST 였다면 06:30 이었을 것입니다. 8절에서 본 자바 `atZone` 의 기본값과 같은 선택입니다.
...
뉴욕에 존재하지 않는 시각이므로 UTC로 바꿀 수가 없습니다. MySQL이 그 삽입을 거부합니다.

**8절의 자바와 반대입니다.** 자바 `atZone` 은 같은 시각을 예외 없이 03:30으로 옮겼습니다. 조용히 한 시간을 옮기는 쪽과 아예 거부하는 쪽이 갈립니다. **같은 값이 계층마다 다르게 처리되고, 어느 쪽도 "이 시각은 없습니다" 라고 사용자에게 말해 주지 않습니다.**

`DATETIME` 은 같은 조건에서 02:30을 그대로 받습니다. 존재하지 않는 시각이 아무 저항 없이 저장됩니다.

**왕복 테스트로는 이 셋 중 아무것도 안 보입니다.** 다른 시간대로 붙은 두 번째 애플리케이션이나 SQL로 직접 읽는 배치가 있어야 드러납니다. 통합 테스트를 앱 한 대로만 짜면 이 구간이 통째로 빠집니다.

## 못 한 것

- **JDBC 왕복은 서버가 UTC 인 조건 하나입니다.** 서버 시간대를 KST로 두고 앱을 뉴욕으로 두는 조합은 안 봤습니다. 그 조합에서는 저장값이 또 달라집니다.
...
- **전환 창 안의 표본이 두 건입니다.** 위 절에서 쓰기가 통째로 멈추는 바람에 창 안에 시도가 두 건밖에 안 들어갔습니다. 락 대기 타임아웃을 넘기는 규모에서 1205로 바뀌는 지점은 재지 않았습니다.
- **초기 회차의 호스트 사양이 없습니다.** 위 절은 macOS 26.3.1, Apple M2 Pro 12코어, 32GB에서 돌았고 그 이전 회차의 장비는 기록이 없습니다.
