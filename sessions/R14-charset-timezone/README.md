...
100만 행 테이블에 초당 50건 남짓의 쓰기를 넣고 `ALGORITHM=COPY` 전환을 걸었습니다.
...
4,124ms 로 `ALTER` 소요 4,193ms 와 거의 같습니다. **전환이 시작될 때 들어온 쓰기는
전환이 끝날 때까지 통째로 멈춥니다.**

창 안의 시도가 2건뿐인 것도 같은 이야기입니다. 기준선에서는 4.2초에 200건 남짓
들어갔을 부하인데, 한 건이 4.1초를 붙잡고 있으니 그 사이 두 건밖에 시작하지 못합니다.

**에러 카운터만 보는 모니터링은 이 4.2초를 정상으로 기록합니다.** `ALGORITHM=COPY` 가
원본에 공유 락을 잡아 읽기는 통과시키고 쓰기만 멈추기 때문에, 조회 기반 헬스체크도
통과합니다. 락 대기 타임아웃(`innodb_lock_wait_timeout`, 기본 50초)보다 `ALTER` 가
...
3절은 자바 안에서 끝났습니다. 실무의 사고는 자바와 서버 사이에서 납니다. 같은 `LocalDateTime` 을 `connectionTimeZone` 만 바꿔 넣고 읽었습니다. JVM 은 UTC, 서버 `time_zone` 은 SYSTEM(UTC) 입니다.

### 왕복만 보면 네 조건이 다 정상입니다
...
넣은 값이 그대로 돌아옵니다. 드라이버가 넣을 때 변환하고 읽을 때 되돌리기 때문입니다. **애플리케이션 하나만 보면 설정이 뭐든 차이가 없습니다.**
...
**중복 시각에서 드라이버가 앞의 것을 고릅니다.** 01:30 이 05:30 으로 저장됐습니다. EDT(UTC-4) 기준입니다. EST 였다면 06:30 이었을 것입니다. 3절에서 본 자바 `atZone` 의 기본값과 같은 선택입니다.
...
**3절의 자바와 반대입니다.** 자바 `atZone` 은 같은 시각을 예외 없이 03:30 으로 옮겼습니다. 조용히 한 시간을 옮기는 쪽과 아예 거부하는 쪽이 갈립니다. **같은 값이 계층마다 다르게 처리되고, 어느 쪽도 "이 시각은 없습니다" 라고 사용자에게 말해 주지 않습니다.**
