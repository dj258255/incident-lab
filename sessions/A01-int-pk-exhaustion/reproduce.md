...
## 실험 3: PostgreSQL 대조 (results/exp3-pg.txt)

```console
$ docker compose up -d postgres
$ ./scripts/exp3-pg.sh      # 약 7분. 300만 행 테이블을 두 벌 만든다
```

원자료: `results/exp3-pg.txt`, 부하 CSV `results/pg-writer-a.csv` / `pg-writer-b.csv`,
청크별 갱신 행 수 `results/pg-backfill-chunks.txt`.

### 밟은 함정 다섯

1. **`docker exec` 에 `-i` 누락.** 히어독으로 부하 스크립트를 컨테이너에 심었는데 표준입력이
   전달되지 않아 파일이 빈 상태로 남았습니다. 에러가 없어서 부하 없이 측정이 끝났고,
   CSV 가 비었다는 것으로만 알아챘습니다. 심은 뒤 줄 수를 확인하는 fail-closed 가드를
   넣었습니다.

2. **DB 컨테이너 안에서 `pkill`.** 부하 루프를 `pkill -f w.sh` 로 끄자 postgres 백엔드가
   SIGTERM 을 받고 postmaster 가 전체를 정리하며 크래시 복구에 들어갔습니다. 3절 값이 전부
   깨졌습니다. 어느 프로세스가 매칭됐는지는 짚지 못했습니다(psql 을 초당 수십 개 포크하는
   동안 PID 재사용과 pkill 의 조회·전송 사이 경합이 유력합니다). 정지 파일을 두고 루프가
   스스로 빠지게 바꿨고, 측정 전후로 `pg_postmaster_start_time()` 을 비교하는 가드를 넣었습니다.

3. **초 단위 타임스탬프로 2초짜리 사건 측정.** `date +%s` 로 센 결과 창 밖의 쓰기가 섞여
   "전환 중에도 초당 20.7건이 통과했다"는 값이 나왔습니다. 밀리초로 바꾸니 5건이었습니다.

4. **비교 대상과 루프 구조가 달랐음.** PostgreSQL 백필을 `WHERE id_new IS NULL` 로만 돌리자
   부하가 방금 넣은 20~30행짜리 청크를 계속 쫓아 수렴하지 않았습니다. 벌크 151회 뒤로
   1,400회 넘게 이어졌고 총 394초가 나왔습니다. MySQL 스크립트는 청크가 2만 미만이면
   벌크를 끊으므로 151회에서 끝납니다. 워터마크로 묶어 구조를 맞췄습니다.

5. **`"$TOTAL행"` 이 변수명 `TOTAL행` 으로 읽힘.** UTF-8 로케일에서 bash 가 한글 바이트를
   이름 문자로 받습니다. `set -u` 때문에 그 줄에서 스크립트가 죽었습니다. `"${TOTAL}행"` 으로
   고쳤고, 저장소 전체를 훑어 A09 와 B01 에서 같은 자리 세 곳을 더 찾아 고쳤습니다.

### 검산

청크별 갱신 행 수를 파일로 남겨 합계가 워터마크와 일치하는지 확인합니다. 테이블 총행 수와
비교하면 백필 중 부하가 넣은 만큼 항상 불일치로 찍히므로, 워터마크와 비교해야 합니다.

```
워터마크 3000203, 청크 갱신 합계 3000203  일치
```
