...
Aurora로 가면 면이 더 넓어집니다. 라이터 엔드포인트뿐 아니라 리더 엔드포인트와 사용자 정의 엔드포인트가 각각 DNS 이름이고, 같은 캐시가 전부에 걸립니다. 리더 쪽에서 같은 캐시가 만드는 다른 사고는 [리더 엔드포인트가 한 인스턴스에만 몰린다](/blog/incident/reader-endpoint-skew)에서 다룹니다.
