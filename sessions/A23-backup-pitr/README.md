...
이 세션은 MySQL에서 시점 복구(PITR)를 실제로 수행하고, 그 과정에서 복구를 가로막는 함정 다섯 가지를 각각 재현합니다. **네 개는 이 세션을 만들면서 제가 직접 밟은 것들입니다.**
...
| 도구 | percona-server:8.4 컨테이너 (`mysqlbinlog` 확보용, 함정 3 참고) |
...
사고 직전의 정답은 1,500건입니다. 백업만 되돌리면 T0 이후가 통째로 사라집니다. **백업 주기가 곧 RPO 상한**이라는 말이 이 500건입니다. Keepthescore가 2020년에 겪은 것이 정확히 이 구조로, 일 1회 백업 덕에 30분 만에 복구했지만 7시간치는 영원히 사라졌습니다.

binlog에서 사고 직전 위치를 찾아 그 구간만 적용하면 유실이 0이 됩니다. 위치는 공식 문서가 안내하는 방식 그대로 찾습니다.

```console
$ mysqlbinlog --base64-output=DECODE-ROWS -v incident.binlog | grep -B3 "DROP TABLE"
# at 176549
#260729  6:12:31 server id 1  end_log_pos 176683 ... Xid = 2700
DROP TABLE `sponsor` /* generated by server */
```

`--stop-position=176549`로 그 직전까지만 적용합니다.

**RTO 분해**: 백업 0.077초 / 덤프 복원 0.085초 / binlog 추출·적용 0.961초. 데이터가 작아 절대값은 의미가 없지만, **복구 시간의 대부분이 binlog 적용**이라는 구조는 규모가 커져도 같습니다. 백업 주기를 늘리면 그만큼 적용할 binlog가 늘어 RTO가 길어집니다. RPO와 RTO는 백업 주기 하나로 맞물려 있습니다.
...
```console
$ docker exec -i mysql mysql -uroot -p < full.sql
...
```console
$ docker exec mysql ls /usr/bin | grep '^mysql'
mysql
mysqladmin
mysqldump
mysqlsh

$ docker exec mysql mysqlbinlog --version
bash: mysqlbinlog: command not found
```

`mysql:8.4` 공식 이미지에 `mysqldump`는 있는데 `mysqlbinlog`가 없습니다. 공식 문서의 PITR 절차는 전부 `mysqlbinlog`로 쓰여 있으므로, 컨테이너로 운영하면서 이 사실을 사고 당일에 알면 복구가 그 자리에서 멈춥니다. 이 세션은 클라이언트 유틸이 들어 있는 percona-server 컨테이너로 우회했습니다.
...
종료 코드 0 (성공)
stderr 없음
재적용 후 행 수 1,500  → 늘어난 행 0건
GTID 차집합(새로 실행된 트랜잭션) ''
```

**명령은 성공하고 에러도 없는데 데이터는 한 건도 들어가지 않습니다.** 공식 문서가 그대로 적어 둔 동작입니다.
