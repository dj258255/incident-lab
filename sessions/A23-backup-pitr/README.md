...
1절의 RTO 분해(백업 / 덤프 복원 / binlog 적용)가 각각 한 번씩 잰 값이었습니다. 어느 구간이 지배하는지로 개선할 곳이 정해지므로, 회차 폭이 구간 사이 차이보다 크면 그 판단이 안 섭니다. 20만 행에 binlog 구간 2만 행으로 5회 돌렸습니다.
...
4. **`mysql:8.4` 이미지에 `mysqlbinlog` 가 없습니다.** 1절이 이미 알고 별도 컨테이너를 쓰고 있었는데 이 스크립트가 그대로 불렀고, `command not found` 가 `2>&1` 에 삼켜졌습니다.
