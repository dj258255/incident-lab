...
전체 스캔이 **2.9배에서 4.9배**입니다. 원인은 버퍼 풀 미스입니다. 순차 PK 는 범위 스캔에서
디스크를 한 번도 때리지 않았고 `uuid7_bin` 은 1,233번 때렸습니다. 테이블이 버퍼 풀보다 크고
페이지가 흩어져 있으면 이어 읽기가 이어 읽기가 되지 못합니다.

단건 조회의 차이는 작습니다(0.069 대 0.121ms). 한 건을 찾는 데는 트리 높이만큼만 읽으면
되기 때문입니다. **랜덤 PK 의 조회 비용은 단건이 아니라 범위와 전체에서 나옵니다.**
...
전체 스캔은 흔들립니다. `uuid4_bin` 이 349.2ms 에서 860.0ms 까지 2.5배 범위입니다.
버퍼 풀 상태와 호스트 부하에 좌우되므로 **절대 시간을 인용하면 안 되고 순차 대비 배수로만
읽어야 합니다.** 그 배수도 2.9배에서 7.1배 사이로 벌어집니다.

범위 스캔 미스는 `seq` 가 3회 모두 0 입니다. **순차 PK 가 이어 읽기에서 디스크를 안 때린다는
것은 편차 없이 성립합니다.**
...
곧 그 페이지에 실린 레코드 바이트 합을 줍니다. 16384 로 나누면 실제 채움 비율입니다.
...
92.1% 가 100% 가 아닌 것은 InnoDB 가 클러스터드 인덱스 페이지의 1/16 을 나중의 행 증가분을
위해 비워 두기 때문입니다. 상한이 약 93.75% 이므로 순차 조건은 사실상 상한에 닿았습니다.
...
`seq` 는 12,261 페이지 중 12,256 이 90~100% 한 칸에 몰려 있습니다. `uuid4_bin` 은
...
## 세컨더리 인덱스 비용과 CHAR(36) 문자셋 분리 (2026-07-31)

### 세컨더리 인덱스가 PK 폭을 그대로 물려받습니다

"InnoDB 의 세컨더리 인덱스는 값으로 PK 를 담으므로 PK 가 넓으면 세컨더리 인덱스도 같이 커집니다" 라고 적어 놓고 얼마나 큰지는 안 쟀습니다. 같은 40만 행에 같은 세컨더리 인덱스 하나(`KEY idx_user (user_id)`)를 붙여 쟀습니다.
...
**인덱스 하나에 3.1배입니다.** UUID 를 `CHAR(36)` 으로 저장하면 세컨더리 인덱스 하나가 BIGINT 일 때의 세 배가 됩니다.

바이트 산술보다 큽니다. `BIGINT` 8바이트에서 `BINARY(16)` 으로 가면 행당 8바이트가 늘어나므로 40만 행에 3.2MB 여야 하는데 실제 증가는 8.0MB 입니다. `CHAR(36)` 은 28바이트가 늘어 11.2MB 여야 하는데 21.1MB 입니다. **둘 다 두 배 남짓입니다.** 인덱스 레코드가 커지면 페이지당 들어가는 레코드가 줄어 페이지 수가 그보다 빨리 늘고, B트리 상위 레벨도 같이 불어나기 때문입니다.

**인덱스가 여러 개면 이 몫이 그만큼 곱해집니다.** 이 세션이 잰 것은 인덱스가 하나도 없는 표입니다. 실제 테이블에 인덱스가 넷이면 PK 폭의 대가가 네 번 붙습니다.

### CHAR(36) 의 대가는 거의 전부 폭입니다

DDL 에 문자셋을 적지 않아 MySQL 8.4 기본값인 utf8mb4 가 걸린 채로 쟀었습니다. 그래서 문자열 저장의 대가 중 폭의 몫과 문자셋의 몫을 가르지 못했습니다. 같은 UUID 값을 세 가지로 저장해 쟀습니다.
...
**폭이 87%, 문자셋이 13% 입니다.** `BINARY(16)` 을 `CHAR(36)` 으로 바꾸는 데 13.2MB 가 들고, 거기서 문자셋을 utf8mb4 로 올리는 데 2.0MB 가 더 듭니다.

문자셋의 몫이 작은 이유는 InnoDB 가 다바이트 문자셋의 `CHAR` 을 **가변 길이로** 저장하기 때문입니다. `CHAR(36)` utf8mb4 는 최대 144바이트지만 실제 값이 16진수 문자열이라 36바이트만 씁니다. 늘어난 2.0MB 는 길이를 적는 몫입니다.

**"utf8mb4 라서 네 배" 는 이 자리에서 안 맞습니다.** 대가는 문자셋이 아니라 16바이트를 36바이트로 늘린 것입니다.

다만 크기가 전부는 아닙니다. 인덱스 접두사 길이 제한은 최대 바이트로 계산하므로 `CHAR(36)` utf8mb4 는 144바이트를 차지하는 것으로 취급됩니다. 복합 인덱스를 만들 때는 그쪽에서 걸립니다.
