...
| 지금 알면 되는 것 | 근거 |
|---|---|
| **UUIDv7 로 바꿨는데 테이블이 안 줄었다.** 밀리초 안 카운터가 없으면 충전율이 UUIDv4 와 같다 | uuid7 54.7% / uuid4 56.7% ([근거](#5-재계측)) |
| **카운터를 넣으면 순차 BIGINT 와 같아진다** | 91.4% 대 91.9% ([근거](#5-재계측)) |
| **대가가 공간과 시간으로 갈라진다.** 카운터 없는 UUIDv7 은 적재 중 디스크 읽기 115쪽, UUIDv4 는 12,613쪽 | ([근거](#6-예상과-달랐던-점)) |
| **세컨더리 인덱스 하나에 3.1배가 붙고, 인덱스가 여러 개면 그만큼 곱해진다** | ([근거](#세컨더리-인덱스-비용과-char36-문자셋-분리)) |
| **"utf8mb4 라서 네 배"는 이 자리에서 안 맞는다.** 폭이 87%, 문자셋이 13% | ([근거](#세컨더리-인덱스-비용과-char36-문자셋-분리)) |
...
## 5-2. 조회 비용과 페이지 충전율 실측

`scripts/read-bench.py`, 실행은 `scripts/run-read.sh`. 원문은 `results/read-*.json`.
조건마다 볼륨을 지우고 새로 적재한 직후에 잽니다. 버퍼 풀 256MB, 120만 행.

### 5.1 적재만 손해가 아니다

| 조건 | 크기 | 단건 p50 | 단건 미스 | 범위 스캔 | 범위 미스 | 전체 스캔 |
|---|---|---|---|---|---|---|
| `seq` | 192MB | 0.069ms | **0** | 193.7ms | **0** | **119.3ms** |
| `uuid4_bin` | 327MB | 0.114ms | 31 | 191.4ms | 298 | 349.2ms |
| `uuid7_bin` | 339MB | 0.121ms | 48 | 209.5ms | **1,233** | 588.3ms |

전체 스캔이 **2.9배에서 4.9배**입니다. 원인은 버퍼 풀 미스입니다. 순차 PK는 범위 스캔에서
디스크를 한 번도 때리지 않았고 `uuid7_bin` 은 1,233번 때렸습니다. 테이블이 버퍼 풀보다 크고
페이지가 흩어져 있으면 이어 읽기가 이어 읽기 구실을 못 합니다.

단건 조회의 차이는 작습니다(0.069 대 0.121ms). 한 건을 찾는 데는 트리 높이만큼만 읽으면
되기 때문입니다. **랜덤 PK의 조회 비용은 단건이 아니라 범위와 전체에서 나옵니다.**

### 반복 측정 3회

`seq` 와 `uuid4_bin` 을 3회 반복했습니다(`results/read-*-run*.json`).

| 조건 | 전체 스캔(3회) | 충전율(3회) | 범위 스캔 미스(3회) |
|---|---|---|---|
| `seq` | 119.3, 117.6, 121.0ms | **92.1, 92.1, 92.1%** | **0, 0, 0** |
| `uuid4_bin` | 349.2, 339.6, **860.0**ms | 66.1, 66.1, 65.4% | 298, 303, 174 |

**충전율은 회차마다 거의 같습니다**(92.1% 셋, 66.1·66.1·65.4%). 페이지 분할이 만드는
공간 손실은 구조가 정하는 값이라 흔들리지 않습니다. **이 세션의 핵심 수치가 안정적이라는
뜻입니다.**

전체 스캔은 흔들립니다. `uuid4_bin` 이 349.2ms에서 860.0ms까지 2.5배 범위입니다.
버퍼 풀 상태와 호스트 부하에 좌우되므로 **절대 시간을 인용하면 안 되고 순차 대비 배수로만
읽어야 합니다.** 그 배수도 2.9배에서 7.1배 사이로 벌어집니다.

범위 스캔 미스는 `seq` 가 3회 모두 0입니다. **순차 PK가 이어 읽기에서 디스크를 안 때린다는
것은 편차 없이 성립합니다.**


### 5.2 페이지 충전율을 직접 쟀다

지금까지 이 세션이 쓴 공간 효율은 `DATA_LENGTH` 를 행 수로 나눈 역산값이었습니다.
`information_schema.INNODB_BUFFER_PAGE` 는 버퍼 풀에 올라온 페이지의 `DATA_SIZE`,
곧 그 페이지에 실린 레코드 바이트 합을 줍니다. 16384로 나누면 실제 채움 비율입니다.

| 조건 | 실측 충전율 | 표본 커버리지 | 페이지당 평균 레코드 |
|---|---|---|---|
| `seq` | **92.1%** | 99.9% | 98.9 |
| `uuid4_bin` | **66.1%** | 77.9% | 67.8 |
| `uuid7_bin` | **63.0%** | 74.9% | 64.6 |

**전수가 아니라 표본입니다.** 버퍼 풀(256MB)이 테이블(192~339MB)보다 작아 모든 페이지가
올라오지 못합니다. 전체 스캔 직후에 읽으므로 최근에 훑은 쪽으로 치우칩니다. 커버리지를
함께 적는 이유이고, 순차 조건만 99.9% 라 조건 사이 비교에도 이 편향이 남습니다.

92.1%가 100%가 아닌 이유는 앞에서 적은 1/16 예약입니다. 상한이 약 93.75% 이므로 순차 조건은 사실상 상한에 닿았습니다.

### 5.3 페이지 분포가 메커니즘을 그대로 보여 준다

| 구간 | `seq` | `uuid4_bin` | `uuid7_bin` |
|---|---|---|---|
| 0~10% | 1 | 124 | **2,770** |
| 10~20% | 0 | 178 | 1,348 |
| 20~30% | 0 | 195 | 1 |
...
| 70~80% | 0 | 2,718 | 2,887 |
| 80~90% | 0 | 2,329 | 0 |
| **90~100%** | **12,256** | 1,503 | 6,342 |

`seq` 는 12,261 페이지 중 12,256이 90~100% 한 칸에 몰려 있습니다. `uuid4_bin` 은
**50~60% 구간에 4,588 페이지**가 쌓여 있고, 이것이 페이지 분할이 만든 반쯤 찬 페이지입니다.
분할은 꽉 찬 페이지를 둘로 쪼개므로 양쪽이 절반씩 남습니다. 그 흔적이 분포로 보입니다.

`uuid7_bin` 이 양극화된 것은 `--uuid7-rows-per-ms 20` 설정 때문입니다. 같은 밀리초 안에서는
순서가 랜덤이라 그 구간에서 분할이 나고, 밀리초가 넘어가면 다시 순차로 채워집니다.
0~10%에 2,770 페이지와 90~100%에 6,342 페이지가 함께 있는 모양이 그것입니다.
...
### 세컨더리 인덱스가 PK 폭을 그대로 물려받습니다

"InnoDB의 세컨더리 인덱스는 값으로 PK를 담으므로 PK가 넓으면 세컨더리 인덱스도 같이 커집니다" 라고 적어 놓고 얼마나 큰지는 안 쟀습니다. 같은 40만 행에 같은 세컨더리 인덱스 하나(`KEY idx_user (user_id)`)를 붙여 쟀습니다.

| PK | 세컨더리 인덱스 크기 | BIGINT 대비 |
|---|---|---|
| `BIGINT` 8바이트 | 10.5MB | 1.00배 |
| `BINARY(16)` | 18.5MB | **1.76배** |
| `CHAR(36)` ascii | 31.6MB | **3.01배** |
| `CHAR(36)` utf8mb4 | 32.6MB | **3.10배** |

**인덱스 하나에 3.1배입니다.** UUID를 `CHAR(36)` 으로 저장하면 세컨더리 인덱스 하나가 BIGINT 일 때의 세 배가 됩니다.

바이트 산술보다 큽니다. `BIGINT` 8바이트에서 `BINARY(16)` 으로 가면 행당 8바이트가 늘어나므로 40만 행에 3.2MB 여야 하는데 실제 증가는 8.0MB입니다. `CHAR(36)` 은 28바이트가 늘어 11.2MB 여야 하는데 21.1MB입니다. **둘 다 두 배 남짓입니다.** 인덱스 레코드가 커지면 페이지당 들어가는 레코드가 줄어 페이지 수가 그보다 빨리 늘고, B트리 상위 레벨도 같이 불어나기 때문입니다.

**인덱스가 여러 개면 이 몫이 그만큼 곱해집니다.** 이 세션이 잰 것은 인덱스가 하나도 없는 표입니다. 실제 테이블에 인덱스가 넷이면 PK 폭의 대가가 네 번 붙습니다.

### CHAR(36)의 대가는 거의 전부 폭입니다

DDL에 문자셋을 적지 않아 MySQL 8.4 기본값인 utf8mb4가 걸린 채로 쟀었습니다. 그래서 문자열 저장의 대가 중 폭의 몫과 문자셋의 몫을 가르지 못했습니다. 같은 UUID 값을 세 가지로 저장해 쟀습니다.

| 저장 방식 | 데이터 크기 | 앞 단계 대비 |
|---|---|---|
| `BINARY(16)` | 109.8MB | 기준 |
| `CHAR(36)` ascii | 123.0MB | **+13.2MB** (폭의 몫) |
| `CHAR(36)` utf8mb4 | 125.0MB | **+2.0MB** (문자셋의 몫) |

**폭이 87%, 문자셋이 13%입니다.** `BINARY(16)` 을 `CHAR(36)` 으로 바꾸는 데 13.2MB가 들고, 거기서 문자셋을 utf8mb4로 올리는 데 2.0MB가 더 듭니다.

문자셋의 몫이 작은 이유는 InnoDB가 다바이트 문자셋의 `CHAR` 을 **가변 길이로** 저장하기 때문입니다. `CHAR(36)` utf8mb4는 최대 144바이트지만 실제 값이 16진수 문자열이라 36바이트만 씁니다. 늘어난 2.0MB는 길이를 적는 몫입니다.

**"utf8mb4라서 네 배" 는 이 자리에서 안 맞습니다.** 대가는 문자셋이 아니라 16바이트를 36바이트로 늘린 것입니다.

다만 크기가 전부는 아닙니다. 인덱스 접두사 길이 제한은 최대 바이트로 계산하므로 `CHAR(36)` utf8mb4는 144바이트를 차지하는 것으로 취급됩니다. 복합 인덱스를 만들 때는 그쪽에서 걸립니다.

## 못 한 것

- **위 두 절은 40만 행에서 조건마다 1회 실행입니다.** 본 실험(500만 행)과 규모가 다르므로 절대값이 아니라 조건 사이의 배수를 봐야 합니다.
- **조회 측정은 세 조건만 했고 반복은 둘만 했습니다.** `uuid7_counter` 와 `uuid4_char` 는 넣지 않았고, `uuid7_bin` 은 1회 실행입니다.
- **충전율이 전수가 아니라 표본입니다.** 버퍼 풀이 테이블보다 작아 커버리지가 74.9~99.9%로 조건마다 다릅니다. 전수를 보려면 `innodb_ruby` 같은 파일 파서를 쓰거나 버퍼 풀을 테이블보다 크게 잡아야 하는데, 후자는 이 세션이 재현하려는 조건 자체를 없앱니다.
