...
충전율은 InnoDB가 직접 알려주지 않으므로 계산한다. 한 행이 쓰는 실제 바이트는 컬럼 크기와
레코드 헤더에서 정해지고, 페이지에 빈 공간이 있으면 DATA_LENGTH를 행 수로 나눈 값이 그보다
커진다. 그 비율이 충전율이다.
...
print("\n두 가지 대가가 따로 논다.")
print("  공간: 페이지가 반만 차면 테이블이 커진다. 충전율 열을 볼 것.")
...
images += [
    {"type": "bar", "out": "01-fill.png", "title": "클러스터드 인덱스 페이지 충전율",
     "bars": fill_bars,
     "note": f"(행 내용 바이트 / 실제 행당 바이트) x 100. 각 조건 {n:,}행 적재 후"},
    {"type": "bar", "out": "02-size.png", "title": "같은 120만 행을 담은 테이블 크기 (MB)",
     "bars": size_bars,
     "note": "페이로드는 네 조건 모두 128바이트로 같다. 차이는 PK 폭과 페이지 충전율에서 온다"},
