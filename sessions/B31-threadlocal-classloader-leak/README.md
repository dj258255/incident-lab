...
여기까지는 웹앱 배포 한 번을 `URLClassLoader` 하나로 흉내 낸 것이었습니다. 이번에는 실제 Tomcat에 서블릿 하나짜리
WAR를 열 번 올렸다 내리며 매번 `/leak` 을 스무 번씩 찔렀습니다. 그 서블릿은
...
**`OOMKilled=true` 인데 `OutOfMemoryError` 가 0회입니다.** JVM은 아무 말도 못 하고
죽었습니다. 스택 트레이스도 힙 덤프도 남지 않습니다. 종료 코드 137은
`128 + 9`, 즉 `SIGKILL` 입니다.
...
**회차 1이 발행된 3,773과 정확히 같습니다.** 회차 2와 3은 워커 스레드에서 죽어
...
GC는 8,000 사이클 동안 누적 13회, 43밀리초만 돌았습니다.
...
- **GC 압박 곡선은 두 조건 1회씩입니다.** 그리고 힙(`-Xmx256m`)이 아니라 Metaspace를 좁힌 조건이라, 힙이 먼저 차는 누수에서는 곡선이 다를 수 있습니다.
Tomcat에서 재배포를 반복해 실제 Metaspace OOM까지 몰고 가지는 않았습니다. 실제 스택 쪽은 언디플로이 1회와 경고 관측까지이고, OOM은 최소 재현에서만 봤습니다. Metaspace OOM은 세 회차 중 한 회차만 정확한 사이클 번호가 남아, 3,773이라는 상수 자체가 반복되는지는 확인하지 않았습니다. 터지는 구간이 재현되는 것은 7절에서 확인했습니다.
