...
매수 쪽은 네 설계가 20.1~30.9ms 로 비슷합니다. 검증을 어차피 해야 하는 경우라
방식의 차이가 작습니다. **D 만 20.1ms 로 눈에 띄는데, `SUM` 대신 한 행을 갱신하기
때문입니다.**

### 물질화 합계는 CHECK 로 막습니다
...
**단일 실행에서 가장 빨랐던 B 가 동시 실행에서 가장 느립니다.** 20.8ms 였던 것이
3,102~6,131ms 입니다. C 도 22.1ms 에서 1,095~2,113ms 로 벌어집니다. 반대로 A 와 D 는
76ms 로 단일 실행 총 소요(34.2ms, 24.9ms)의 2~3배에 그칩니다.

**이 역전의 원인은 짚지 못했습니다.** 매도만 던지므로 B 의 `WHEN` 절과 C 의 델타 검사가
둘 다 검증을 걸러 내야 하고, 그러면 advisory lock 도 안 잡혀야 합니다. 그런데도 느립니다.
...
있습니다. D 는 두 축에서 모두 나쁘지 않은데, 대가는 모든 입출고가 `company` 한 행을
갱신한다는 점입니다. 이 실험의 부하(네 세션)에서는 그 직렬화가 아직 병목이 아닙니다.

### 이 절의 한계

조건마다 1회 실행이고 동시성은 네 세션 하나입니다. 세션 수를 늘리면 D 의 직렬화가
