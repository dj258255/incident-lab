...
| 지금 알면 되는 것 | 근거 |
|---|---|
| **행 단위 CHECK 는 잔고 합계를 막지 못한다.** 발행총수의 30.66배 착오 입고가 제약에 걸리지 않고 커밋된다 | ([근거](#2-재현)) |
| **총량 검증 트리거와 maker-checker 가 이것을 막는다** | ([근거](#4-해소)) |
| **그런데 트리거도 동시 입고 2세션이면 뚫린다.** 각자는 한도 안이고 합쳐야 넘는다 | ([근거](#6-예상과-달랐던-점)) |
| **직렬화까지 가야 닫힌다.** 설계 넷의 비용이 각각 다르다 | ([근거](#설계-넷의-비용과-직렬화)) |
...
`sql/08-designs.sql` 과 `scripts/run-designs.sh`, 결과는 `results/designs.txt` 입니다.

네 설계를 같은 원장에서 나란히 잽니다.

- **A.** 현재 설계. 문장 단위 트리거 + advisory lock. 매도든 매수든 전체 `SUM` 을 돈다
- **B.** 행 단위 트리거 + `WHEN (NEW.shares > OLD.shares)`. 늘어난 행에서만 검증
- **C.** 전이 테이블(`REFERENCING OLD/NEW TABLE`). 문장 단위이면서 증감을 구분
- **D.** `company` 에 물질화한 합계 컬럼 + `CHECK (total_shares <= issued_shares)`

### 증감을 구분하면 매도가 싸집니다

200회씩 1회 실행입니다.

| 설계 | 매도(합계 감소) | 매수(합계 증가) | 매도 A 대비 |
|---|---|---|---|
| A. 문장 단위 + advisory lock | 34.2ms | 30.2ms | 1.00 |
| B. 행 단위 + `WHEN` | **20.8ms** | 30.9ms | **0.61** |
| C. 전이 테이블 | 22.1ms | 26.2ms | 0.65 |
| D. 물질화 합계 + CHECK | 24.9ms | 20.1ms | 0.73 |

**매도가 35~39% 싸집니다.** 매도는 합계를 줄이므로 검증이 필요 없는데 현재 설계는
매번 전체 `SUM` 을 돕니다. `WHEN` 절이나 전이 테이블로 걸러 내면 그 스캔이 사라집니다.

매수 쪽은 네 설계가 20.1~30.9ms로 비슷합니다. 검증을 어차피 해야 하는 경우라
방식의 차이가 작습니다. **D만 20.1ms로 눈에 띄는데, `SUM` 대신 한 행을 갱신하기
때문입니다.**

### 물질화 합계는 CHECK로 막습니다

D 설계에 착오 배당 배치를 던졌습니다.

```
거부됨(CHECK 위반): new row for relation "company" violates check constraint
"chk_total_within_issued"
```

**교차행 불변식이 한 행 안으로 들어와 `CHECK` 범위가 됩니다.** 3절이 "`CHECK` 로는
표현할 수 없다"고 적은 것은 `holdings` 여러 행에 걸친 합계를 그 테이블의 `CHECK` 로
쓰려 할 때이고, 합계를 `company` 한 행에 물질화하면 표현이 가능해집니다.

### 동시 매도에서는 순서가 뒤집힙니다

네 세션이 각각 매도 50회를 던집니다. 총 200회로 위 표와 같은 양입니다.

| 설계 | 가장 빠른 세션 | 가장 느린 세션 | 실패 |
|---|---|---|---|
| A. 문장 단위 + advisory lock | **76ms** | 103ms | 0 |
| B. 행 단위 + `WHEN` | **3,102ms** | **6,131ms** | 0 |
| C. 전이 테이블 | 1,095ms | 2,113ms | 0 |
| D. 물질화 합계 + CHECK | **76ms** | 96ms | 0 |

**단일 실행에서 가장 빨랐던 B가 동시 실행에서 가장 느립니다.** 20.8ms 였던 것이
3,102~6,131ms입니다. C도 22.1ms에서 1,095~2,113ms로 벌어집니다. 반대로 A와 D는
76ms로 단일 실행 총 소요(34.2ms, 24.9ms)의 2~3배에 그칩니다.

이 역전의 원인은 짚지 못했습니다. 매도만 던지므로 B의 `WHEN` 절과 C의 델타 검사가
둘 다 검증을 걸러 내야 하고, 그러면 advisory lock도 안 잡혀야 합니다. 그런데도 느립니다.
행 단위 트리거의 행별 평가 비용이나 전이 테이블을 만드는 비용이 동시 실행에서
드러나는 것으로 의심하지만 확인하지 않았습니다. 관측만 적고 원인은 열어 둡니다.

말할 수 있는 것은 이것입니다. **단일 문장 비용으로 설계를 고르면 안 됩니다.**
증감 구분이 매도 한 건을 39% 싸게 만들지만, 동시 부하에서는 그 이득이 뒤집힐 수
있습니다. D는 두 축에서 모두 나쁘지 않은데, 대가는 모든 입출고가 `company` 한 행을
갱신한다는 점입니다. 이 실험의 부하(네 세션)에서는 그 직렬화가 아직 병목이 아닙니다.

### 이 절의 한계

조건마다 1회 실행이고 동시성은 네 세션 하나입니다. 세션 수를 늘리면 D의 직렬화가
언제 병목이 되는지 보이는데 그 조건은 만들지 않았습니다.
