...
39세션 중 SQL Server 를 직접 만진 아홉을 한 줄씩 추린 **[SQLSERVER.md](SQLSERVER.md)** 가 있습니다.
...
<!-- 세션이 완료되면 아래에 추가 -->
- [A27 옵티마이저가 조인 연산자를 고르는 기준을 재봤다](sessions/A27-mssql-physical-join/) — 주문 100만·등급 1천에서 Nested Loops·Merge·Hash 를 차례로 강제해 쟀다. 조인 키에 인덱스가 있으면 Merge 가 메모리를 0 쓰고(정렬 연산자가 안 붙는다) 인덱스를 빼면 3,968KB 를 요구한다. 같은 데이터에서 인덱스 유무가 논리 읽기를 8 대 3,119 로 가른다. 논리 읽기가 셋 다 3,119 로 같은데 OPTION (MERGE JOIN) 은 69,840KB 를 요구해 Hash 의 17배다. 5~10% 구간에서는 추정이 정확한데도 옵티마이저가 논리 읽기 기준 최적을 안 고르고(3,317 대 3,119), 통계를 낡게 만들어 추정이 408배 어긋나게 해도 연산자는 안 뒤집혔다. 기본 CE 가 히스토그램 밖 값을 밀도 벡터로 잡아 준다
...
- [A30 재화를 한 숫자로 두면 못 하는 대답들](sessions/A30-currency-paid-free-split/) — A24·A25 는 재화를 balance 한 컬럼으로 다뤘다. 회수는 그것으로 됐는데, 회수 말고 다른 질문이 들어오면 어디까지 답하는가. 같은 이력을 세 모델(단일 잔액 / 유상·무상 두 컬럼 / 로트)에 담으니 총 잔액은 25,333,000 으로 셋이 같다. 갈리는 것은 답할 수 있는 질문의 수다 — 미사용 유료 재화, 주문별 미사용분, 이 소모가 온전히 무상이었나, 만료분만 소멸, 소모 순서별 환불 노출액 다섯 중 **단일 잔액은 하나만 답한다**. 공정위 의결 2024-005 가 "유상 구입한 것인지 무상 획득한 것인지 구별이 불가능"하다고 적은 상태가 바로 그 모델이고, 그래서 관련매출액 산정 자체가 안 돼 정액과징금 7,000만 원으로 처리됐다(유상 위주였던 쪽은 116억). 소모 순서만 뒤집어도 환불 노출액이 23,333,000 에서 19,333,000 으로 소모 총액만큼 갈린다 — 정책이 곧 재무 리스크다
- [A29 승격하면 무엇을 잃는가](sessions/A29-mssql-log-shipping-failover/) — A23 의 MySQL 반동기는 강등을 모른 채 승격해 커밋 334건을 잃었다. 같은 질문을 SQL Server 로그 전달에 물었다. 두 라운드 전달에서 보조가 10,005 → 10,010 을 따라오고, 아직 백업 안 된 3행이 보조에 없다 — 그 차이가 RPO 다. 꼬리 로그를 `WITH NORECOVERY` 로 뜨는 순간 주가 RESTORING 으로 내려가 쓰기를 거부하고, 보조가 그 꼬리까지 복원해 열면 10,013행으로 장애 직전 주와 정확히 같다(유실 0). 유실을 정하는 것은 기종이 아니라 절차였다. 그리고 승격이 끝나도 앱은 못 들어온다 — 사용자는 DB 를 따라오는데 로그인은 master 에 남아, 같은 이름으로 만들어도 SID 가 달라 고아가 된다(user 0x6C0C… vs login 0x4765…). `ALTER USER ... WITH LOGIN` 한 줄이 수리고, 예방은 이관 때 `WITH SID` 로 만드는 것이다. 페일오버 절차서의 마지막 줄은 DB 가 아니라 로그인이다
- [A28 손상은 조용하고, 복구는 체인이 정한다](sessions/A28-mssql-corruption-page-restore/) — 백업 검증(A23)이 "백업이 온전한가"였다면 이번 질문은 "지금 도는 DB 가 온전한가"다. `.mdf` 의 데이터 페이지 한 장(1:450)에서 32바이트를 0으로 덮고 ONLINE 으로 되돌려도 아무 오류가 없다. 그 페이지를 읽는 순간 824 로 죽고(`incorrect checksum (expected: 0x1124b6cf, actual: 0x229cb5ef)`), 엔진이 `msdb.dbo.suspect_pages` 에 event_type 2 로 스스로 적는다. `RESTORE DATABASE ... PAGE='1:450'` 으로 그 한 장만 되돌리고 로그와 꼬리 로그를 재생하니 백업 이후에 쓴 10행까지 살아서 손실 0, suspect_pages 는 2 → 4(복원됨)로 전이. 같은 손상에 `REPAIR_ALLOW_DATA_LOSS` 를 쓰면 CHECKDB 는 깨끗해지지만 30,000행이 29,923행이 된다 — 77행이 사라지고 어디에도 안 남는다. 두 길의 차이는 로그 체인 하나다
- [A26 느려졌다는 신고를 받고 원인을 좁히는 순서](sessions/A26-mssql-diagnosis/) ([운영 절차서](sessions/A26-mssql-diagnosis/RUNBOOK.md)) — 대기 통계는 인스턴스가 뜬 뒤로 누적이라 같은 LCK_M_IS 가 누적 430,521ms, 이번 사고가 만든 것은 107,453ms 로 네 배 차이. 두 번 떠서 차분으로 봐야 하고, 유휴 대기(SOS_WORK_DISPATCHER 등)를 무시 목록에서 빠뜨리면 차분을 떠도 신호가 묻힌다. 블로킹은 blocking_session_id 를 그대로 보면 중간 세션이 나오므로 재귀로 사슬(53←52←51)을 따라 뿌리까지 가야 하고, **그 뿌리는 sleeping 이라 dm_exec_requests 에 안 나온다** — 요청은 없는데 트랜잭션은 열린 세션으로 찾는다. 락이 아닐 때 보는 계획 캐시도 계획 단위로 정렬하면 리터럴 쿼리가 60개로 흩어져 안 잡히고, query_hash 로 묶어야 실제 부하가 151,326 에서 304,086 으로 드러난다
- [A25 이미 써 버린 재화를 어떻게 회수하는가](sessions/A25-currency-reclaim-procedure/) ([운영 절차서](sessions/A25-currency-reclaim-procedure/RUNBOOK.md)) — 회수 대상 6만 계정 중 2만은 잔액이 회수액보다 적다. CHECK 제약이 Msg 547 로 회수를 막고, 음수 잔액과 빚 컬럼 두 설계가 회수액 1,839,974,000 으로 같되 상계 방식이 갈린다. 4,000행 배치로 승격 0회, 다섯째 배치에서 죽여도 같은 batch_id 로 이어 돌려 이중 회수 0·누락 0, 동시 사용 중에도 회수액이 대상과 정확히 일치. 잘못된 목록으로 회수한 뒤 BEGIN TRAN WITH MARK 지점으로 복원해 잔액 합계가 보정 전과 일치. A24 의 조사 DB에서 bcp로 회수 대상을 넘겨 실제로 이었고, 파일이 절반에서 잘리면 프로시저 검산은 통과하고(30건 받아 30건 회수) 원본과의 건수·합계·체크섬 대조만 잡는다. 승인 절차는 권한(Msg 229)과 로직(자기 승인 거부) 두 층으로 막고, 승인 뒤 대상 표에 행을 넣는 바꿔치기는 승인 시점 지문 대조로 차단. 잘못된 보정은 운영 DB 를 통째로 되돌리지 않고(그러면 보정 뒤 정상 활동 3억이 사라진다) 복원본에서 원래 값을 읽어 되돌리는 보정을 한 번 더 돈다
- [A24 셀 수 없으면 롤백 말고는 선택지가 없다](sessions/A24-currency-anomaly-detection/) ([운영 절차서](sessions/A24-currency-anomaly-detection/RUNBOOK.md)) — 재화 원장 2,000만 행에서 이상 지급 탐지 3안을 정답지와 대조. 참조 대사와 개봉 대사는 이상 계정 60개를 오탐 0으로 적발, 통계 이탈은 같은 60개를 다 잡으면서 정상 헤비 이용자 40개를 함께 지목해 오회수를 만든다. 회수 대상을 계정·금액까지 정확히 산정(360,000). 조사 쿼리 논리 읽기는 인덱스 없이 229,614인데 조건 컬럼 reason이 빠진 640MB 인덱스는 228,399로 그대로였고, reason을 넣자 496, 필터드는 크기 절반에 482. 필터드는 조회 쪽 SET 옵션이 안 맞으면 에러 없이 무시돼 229,614로 돌아간다. 그 인덱스를 사고 중에 만들면 막히는 것은 조회가 아니라 쓰기다(오프라인 빌드는 S 락이라 INSERT 가 13.7초 대기, ONLINE=ON 은 IS 락이라 0회). 참조가 안 남는 두 번째 사고에서는 참조·개봉 대사가 0/40 이고 통계 이탈만 40/40 이라, 오탐 때문에 회수 근거로 못 쓰는 그 방법을 버릴 수도 없다. 쏠린 분포에서는 5시그마까지 올려도 지목된 계정의 53.8%가 무고하고, 파티셔닝은 조사를 안 빠르게 했다(638 대 657) — 창을 좁힌 것은 클러스터드 키를 시간 순으로 잡은 쪽이었다
- [A04 5,000이라고 적힌 임계값이 6,250에서 발동했다](sessions/A04-mssql-lock-escalation/) ([운영 절차서](sessions/A04-mssql-lock-escalation/RUNBOOK.md)) — 문서가 말하는 5,000행에서 승격이 안 일어나고 이분 탐색으로 찾은 경계는 테이블 락 6,249개(확장 이벤트가 적은 escalated_lock_count 와 일치). 승격하면 보정 대상이 아닌 계정 조회가 18.5초 정지(LCK_M_IS), 4,000행 배치 분할은 승격 0회에 정지 0회. 유지 락은 5개 대 4,015개 대 200,548개. 규모를 40만행으로 키우면 엔진이 페이지 락을 골라 승격 자체가 사라지고, 통계가 없으면 같은 6,231행이 승격. 갱신 컬럼에 보조 인덱스가 하나 붙으면 행당 KEY 락이 2개 더 붙어(옛 자리 삭제 + 새 자리 삽입) 경계가 2,907행으로 내려가므로, 배치 처방은 인덱스 수를 세고 정해야 함. 옆 세션이 락을 들면 승격이 실패해 행 락 2만 개를 끝까지 들고 가고(옆 세션이 커밋하면 다음 재시도가 성공), 파티션 테이블에서는 AUTO 가 파티션 단위로 승격해 그 밖을 안 막지만 기본값 TABLE 은 전체를 잠그고, 대신 AUTO 만 데드락이 3/3 난다. 재시도 루프를 넣으면 3/6 이 6/6 이 되고 네 조건 모두 결과 정합이 어긋나지 않는데(재시도가 트랜잭션 전체를 다시 하므로), 정작 중요한 것은 우선순위였다 — 안 주면 엔진이 희생자를 고르고 9회 모두 살리고 싶었던 게임 트래픽 쪽이 죽었다. 엔진의 기준은 되돌리는 비용이지 서비스 영향이 아니다. 확장 이벤트의 hobt_lock_count 를 보면 규칙이 나온다 — 1,250개마다 검사하고 락 5,000개를 넘은 인덱스를 승격시킨다. 문서의 5,000 은 문장 전체가 아니라 인덱스 하나를 가리키는 값이었다
