...
- [A04 5,000이라고 적힌 임계값이 6,250에서 발동했다](sessions/A04-mssql-lock-escalation/) — 문서가 말하는 5,000행에서 승격이 안 일어나고 이분 탐색으로 찾은 경계는 테이블 락 6,249개(확장 이벤트가 적은 escalated_lock_count 와 일치). 승격하면 보정 대상이 아닌 계정 조회가 18.5초 정지(LCK_M_IS), 4,000행 배치 분할은 승격 0회에 정지 0회. 유지 락은 5개 대 4,015개 대 200,548개. 규모를 40만행으로 키우면 엔진이 페이지 락을 골라 승격 자체가 사라지고, 통계가 없으면 같은 6,231행이 승격
